...
/**
 * Rappresenta un piano di tratta composto da fermate ordinate. Esempio: partenza, ritiro, consegna,
 * arrivo.
 */
public final class RoutePlan {
  ...
  private RoutePlan(
      String routeNumber, List<RouteStop> stops, Distance estimatedDistance, Notes notes) {
  ...
  private static String validateRouteNumber(String routeNumber) {
    ...
    if (normalizedRouteNumber.length() > MAX_ROUTE_NUMBER_LENGTH) {
      throw new IllegalArgumentException(
          "Il numero tratta non può superare " + MAX_ROUTE_NUMBER_LENGTH + " caratteri.");
    }

    if (!normalizedRouteNumber.matches("[A-Z0-9_-]+")) {
      throw new IllegalArgumentException(
          "Il numero tratta può contenere solo lettere, numeri, trattini e underscore.");
    }
  ...
  private static List<RouteStop> validateAndNormalizeStops(List<RouteStop> stops) {
    ...
    if (stops.stream().anyMatch(Objects::isNull)) {
      throw new IllegalArgumentException(
          "La lista delle fermate non può contenere elementi nulli.");
    }

    List<RouteStop> orderedStops =
        stops.stream().sorted(Comparator.comparingInt(RouteStop::getSequenceNumber)).toList();
    ...
    for (int i = 0; i < orderedStops.size(); i++) {
      ...
      if (orderedStops.get(i).getSequenceNumber() != expectedSequenceNumber) {
        throw new IllegalArgumentException(
            "Le fermate devono avere numeri di sequenza progressivi a partire da 1.");
      }
    }
  ...
  public List<RouteStop> getCargoOperationStops() {
    return stops.stream().filter(RouteStop::isCargoOperation).toList();
  }
