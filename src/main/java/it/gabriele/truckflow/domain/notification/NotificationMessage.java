...
public final class NotificationMessage {
    ...
    private static String validateNotificationNumber(String notificationNumber) {
        ...
        if (normalizedNotificationNumber.length() > MAX_NOTIFICATION_NUMBER_LENGTH) {
            throw new IllegalArgumentException("Il numero notifica non può superare "
                    + MAX_NOTIFICATION_NUMBER_LENGTH + " caratteri.");
        }

        if (!normalizedNotificationNumber.matches("[A-Z0-9_-]+")) {
            throw new IllegalArgumentException("Il numero notifica può contenere solo lettere, numeri, trattini e underscore.");
        }
    ...
    private static String validateRecipientReference(String recipientReference) {
        ...
        if (normalizedRecipientReference.isEmpty()) {
            throw new IllegalArgumentException("Il riferimento destinatario notifica non può essere vuoto.");
        }

        if (normalizedRecipientReference.length() > MAX_RECIPIENT_REFERENCE_LENGTH) {
            throw new IllegalArgumentException("Il riferimento destinatario notifica non può superare "
                    + MAX_RECIPIENT_REFERENCE_LENGTH + " caratteri.");
        }

        if (!normalizedRecipientReference.matches("[A-Za-z0-9@._+\\-]+")) {
            throw new IllegalArgumentException("Il riferimento destinatario contiene caratteri non validi.");
        }
    ...
    private static void validateSchedule(
        ...
        if (status == NotificationStatus.SCHEDULED && scheduledAt == null) {
            throw new IllegalArgumentException("Una notifica programmata deve avere una data di pianificazione.");
        }
        ...
        if (scheduledAt != null && sentAt != null && sentAt.isBefore(scheduledAt)) {
            throw new IllegalArgumentException("La data invio notifica non può essere precedente alla data pianificata.");
        }
    }
    ...
    public String formatSingleLine() {
        return notificationNumber
                + " - " + type
                + " - " + recipientType + ":" + recipientReference
                + " - " + channel
                + " - " + status;
    }
