...
public final class CargoLoad {
  ...
  public Weight calculateTotalWeight() {
    double totalKilograms =
        items.stream().mapToDouble(item -> item.getWeight().getKilograms()).sum();
  ...
  public Volume calculateTotalVolume() {
    double totalCubicMeters =
        items.stream().mapToDouble(item -> item.calculateVolume().getCubicMeters()).sum();
  ...
  public boolean allItemsFitInside(Dimension cargoSpaceDimension) {
    ...
    return items.stream().allMatch(item -> item.getDimension().fitsInside(cargoSpaceDimension));
  }
