...
public final class ShipmentDocumentBundle {
  ...
  private ShipmentDocumentBundle(
    ...
    this.shipmentNumber =
        validateCode(shipmentNumber, "Il numero spedizione fascicolo è obbligatorio.");
  ...
  private static Set<TransportDocumentType> validateRequiredTypes(
      Set<TransportDocumentType> requiredTypes) {
  ...
  private static String validateCode(String code, String message) {
    ...
    if (normalized.length() > MAX_CODE_LENGTH) {
      throw new IllegalArgumentException(
          "Il codice fascicolo non può superare " + MAX_CODE_LENGTH + " caratteri.");
    }
    if (!normalized.matches("[A-Z0-9_-]+")) {
      throw new IllegalArgumentException(
          "Il codice fascicolo può contenere solo lettere, numeri, trattini e underscore.");
    }
    return normalized;
  }
  ...
  public Set<TransportDocumentType> getRequiredTypes() {
    return requiredTypes;
  }

  public List<TransportDocument> getDocuments() {
    return documents;
  }
