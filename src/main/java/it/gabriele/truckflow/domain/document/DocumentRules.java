...
public final class DocumentRules {
  ...
  public static boolean canBeExpired(TransportDocument document) {
    ...
    return document.getStatus() == DocumentStatus.VERIFIED && document.hasExpirationDate();
  }

  public static boolean isExpiredOn(TransportDocument document, LocalDate referenceDate) {
    validateDocument(document);
    ...
    return document.hasExpirationDate() && referenceDate.isAfter(document.getExpirationDate());
  }

  public static boolean isValidForOperation(TransportDocument document, LocalDate referenceDate) {
    validateDocument(document);

    return document.isVerified() && !isExpiredOn(document, referenceDate);
  }
  ...
  public static boolean allDocumentsValidForOperation(
      List<TransportDocument> documents, LocalDate referenceDate) {
    validateDocuments(documents);
    ...
    return documents.stream().allMatch(document -> isValidForOperation(document, referenceDate));
  }
