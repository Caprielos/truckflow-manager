...
public final class TrackingTimeline {
    ...
    private TrackingTimeline(List<TrackingEvent> events) {
        ...
        if (events.stream().anyMatch(Objects::isNull)) {
            throw new IllegalArgumentException("La lista eventi tracking non può contenere valori nulli.");
        }
    ...
    private static void validateSameMissionAndShipment(List<TrackingEvent> events) {
        ...
        boolean allSameMission = events.stream()
                .allMatch(first::isSameMission);

        boolean allSameShipment = events.stream()
                .allMatch(first::isSameShipment);

        if (!allSameMission || !allSameShipment) {
            throw new IllegalArgumentException("Tutti gli eventi tracking devono appartenere alla stessa missione e spedizione.");
        }
    }

    private static void validateUniqueEventCodes(List<TrackingEvent> events) {
        long uniqueCodes = events.stream()
                .map(TrackingEvent::getEventCode)
                .distinct()
                .count();
    ...
    public boolean containsEventCode(String eventCode) {
        ...
        return events.stream()
                .anyMatch(event -> event.getEventCode().equals(normalizedEventCode));
    }
