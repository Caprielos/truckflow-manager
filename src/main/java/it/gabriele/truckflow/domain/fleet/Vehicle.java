...
/**
 * Rappresenta un veicolo fisico della flotta. Può essere un furgone, camion, trattore stradale o
 * rimorchio.
 */
public final class Vehicle {
  ...
  private Vehicle(
    ...
    this(
        ...
        chassisNumber,
        type,
        type == null ? null : type.getUnitType(),
        bodyType,
        null,
        status,
        tireSpecification,
  ...
  public static Vehicle technicalVehicle(
    ...
    VehicleType legacyType =
        legacyTypeFrom(
            unitType, technicalSpecification.getBodyConfiguration().supportsTemperatureControl());
    VehicleBodyType legacyBodyType =
        legacyBodyTypeFrom(technicalSpecification.getBodyConfiguration().getBaseType());
    Weight payload = technicalSpecification.getMassSpecification().calculateNetPayload();
    Dimension cargoDimension =
  ...
  private static String validateFleetNumber(String fleetNumber) {
    ...
    if (normalizedFleetNumber.length() > MAX_FLEET_NUMBER_LENGTH) {
      throw new IllegalArgumentException(
          "Il numero flotta non può superare " + MAX_FLEET_NUMBER_LENGTH + " caratteri.");
    }

    if (!normalizedFleetNumber.matches("[A-Z0-9_-]+")) {
      throw new IllegalArgumentException(
          "Il numero flotta può contenere solo lettere, numeri, trattini e underscore.");
    }
  ...
  private static String validateLicensePlate(String licensePlate) {
    ...
    if (normalizedLicensePlate.length() > MAX_LICENSE_PLATE_LENGTH) {
      throw new IllegalArgumentException(
          "La targa non può superare " + MAX_LICENSE_PLATE_LENGTH + " caratteri.");
    }

    if (!normalizedLicensePlate.matches("[A-Z0-9 -]+")) {
      throw new IllegalArgumentException(
          "La targa può contenere solo lettere, numeri, spazi e trattini.");
    }
  ...
  private static String validateChassisNumber(String chassisNumber) {
    ...
    if (normalizedChassisNumber.length() != VIN_LENGTH) {
      throw new IllegalArgumentException(
          "Il numero telaio deve avere " + VIN_LENGTH + " caratteri.");
    }

    if (!normalizedChassisNumber.matches("[A-HJ-NPR-Z0-9]+")) {
      throw new IllegalArgumentException(
          "Il numero telaio può contenere solo lettere e numeri, esclusi I, O e Q.");
    }
  ...
  private static void validateBodyType(VehicleType type, VehicleBodyType bodyType) {
    ...
    if (type.supportsTemperatureControl() && !bodyType.supportsTemperatureControl()) {
      throw new IllegalArgumentException(
          "Un veicolo refrigerato deve avere un allestimento refrigerato.");
    }
  }
  ...
  private static void validateCargoData(
    ...
    if (type.canCarryCargo()) {
      if (maxPayload == null) {
        throw new IllegalArgumentException(
            "La portata massima è obbligatoria per un veicolo cargo.");
      }

      if (cargoSpaceDimension == null) {
        throw new IllegalArgumentException(
            "Le dimensioni dello spazio di carico sono obbligatorie per un veicolo cargo.");
      }
  ...
  private static void validateTemperatureData(
      VehicleType type, VehicleBodyType bodyType, TemperatureRange temperatureRange) {
    boolean requiresTemperatureRange =
        type.supportsTemperatureControl() || bodyType.supportsTemperatureControl();

    if (requiresTemperatureRange && temperatureRange == null) {
      throw new IllegalArgumentException(
          "Un veicolo a temperatura controllata deve avere un intervallo di temperatura.");
    }

    if (!requiresTemperatureRange && temperatureRange != null) {
      throw new IllegalArgumentException(
          "Un veicolo non refrigerato non può avere un intervallo di temperatura.");
    }
  }

  private static void validateTechnicalSpecification(
      VehicleUnitType unitType, VehicleTechnicalSpecification technicalSpecification) {
    ...
    if (!unitType.canCarryCargo() && bodyConfiguration.isCargoBody()) {
      throw new IllegalArgumentException(
          "Un'unità non cargo può avere accessori, ma non un allestimento base cargo.");
    }

    if (unitType.canCarryCargo() && !bodyConfiguration.isCargoBody()) {
      throw new IllegalArgumentException(
          "Un'unità cargo deve avere una configurazione allestimento cargo.");
    }

    if (unitType == VehicleUnitType.TRACTOR_UNIT
        && technicalSpecification.getCouplingSpecification().getCouplingType()
            != CouplingType.FIFTH_WHEEL) {
      throw new IllegalArgumentException(
          "Un trattore stradale deve avere dati ralla/quinta ruota.");
    }
    ...
    if ((unitType == VehicleUnitType.DRAWBAR_TRAILER
  ...
  private static VehicleType legacyTypeFrom(
      VehicleUnitType unitType, boolean temperatureControlled) {
    return switch (unitType) {
      case VAN -> VehicleType.VAN;
      case RIGID_TRUCK ->
          temperatureControlled ? VehicleType.REFRIGERATED_TRUCK : VehicleType.RIGID_TRUCK;
      ...
      case SEMI_TRAILER ->
          temperatureControlled ? VehicleType.REFRIGERATED_TRAILER : VehicleType.SEMI_TRAILER;
    };
  }
  ...
  public boolean canFitDimension(Dimension dimension) {
    ...
    Dimension availableCargoSpace =
        technicalSpecification != null
            ? technicalSpecification.getDimensionSpecification().getCargoSpaceDimension()
            : cargoSpaceDimension;
    return canCarryCargo()
        && availableCargoSpace != null
        && dimension.fitsInside(availableCargoSpace);
  }
  ...
  public boolean canSupportTemperatureRange(TemperatureRange requiredTemperatureRange) {
    ...
    return supportsTemperatureControl() && requiredTemperatureRange.isCoveredBy(temperatureRange);
  }
