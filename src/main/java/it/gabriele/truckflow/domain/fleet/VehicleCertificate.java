...
public final class VehicleCertificate {
  ...
  private VehicleCertificate(
      VehicleCertificateType type, LocalDate validFrom, LocalDate expiresAt, Notes notes) {
    if (type == null) {
    ...
    if (validFrom != null && validFrom.isAfter(expiresAt)) {
      throw new IllegalArgumentException(
          "La data inizio validità non può essere successiva alla scadenza.");
    }
    if (notes == null) {
  ...
  public static VehicleCertificate of(
      VehicleCertificateType type, LocalDate validFrom, LocalDate expiresAt, Notes notes) {
