...
public final class VehicleBodyCompatibilityRules {
  ...
  private static boolean isBodyCompatibleWithCargoItem(VehicleBodyType bodyType, CargoItem item) {
    ...
    return switch (category) {
      ...
      case REFRIGERATED_FOOD, PHARMACEUTICAL, TEMPERATURE_CONTROLLED_GOODS ->
          isTemperatureControlledBody(bodyType);
      ...
      case VEHICLES ->
          bodyType == VehicleBodyType.CAR_TRANSPORTER
              || bodyType == VehicleBodyType.FLATBED
              || bodyType == VehicleBodyType.LOW_LOADER;
      case LIQUID ->
          bodyType == VehicleBodyType.TANK_LIQUID || bodyType == VehicleBodyType.FOOD_GRADE_TANK;
      case FOOD_GRADE_LIQUID ->
          bodyType == VehicleBodyType.FOOD_GRADE_TANK || bodyType == VehicleBodyType.TANK_LIQUID;
      case FUEL -> bodyType == VehicleBodyType.TANK_FUEL;
      case GAS -> bodyType == VehicleBodyType.TANK_GAS;
      case CONSTRUCTION_MATERIAL, BULK_INERT_GOODS, CONCRETE ->
          bodyType == VehicleBodyType.FLATBED
      ...
      case BULK_DRY, AGRICULTURAL_BULK, HAY_BALES ->
          bodyType == VehicleBodyType.SILO
              ...
              || bodyType == VehicleBodyType.THREE_WAY_TIPPER
              || bodyType == VehicleBodyType.FLATBED;
      case WASTE_NON_DANGEROUS, WASTE_DANGEROUS ->
          bodyType == VehicleBodyType.WALKING_FLOOR
              || bodyType == VehicleBodyType.TIPPER
              || bodyType == VehicleBodyType.REAR_TIPPER
              || bodyType == VehicleBodyType.THREE_WAY_TIPPER
  ...
  private static boolean containsGasDangerousGoods(CargoLoad cargoLoad) {
    return cargoLoad.getDangerousGoodsProfiles().stream().anyMatch(DangerousGoodsProfile::isGas);
  }

  private static boolean containsFuelDangerousGoods(CargoLoad cargoLoad) {
    return cargoLoad.getDangerousGoodsProfiles().stream()
        .anyMatch(profile -> profile.isUnNumber("UN 1202") || profile.isUnNumber("UN 1203"));
  }
