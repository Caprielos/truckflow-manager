...
public final class TireSpecification {
  ...
  private TireSpecification(
      String brand, String model, String size, int loadIndex, String speedRating) {
    this.brand = validateText(brand, "La marca dello pneumatico è obbligatoria.", MAX_BRAND_LENGTH);
    this.model =
        validateText(model, "Il modello dello pneumatico è obbligatorio.", MAX_MODEL_LENGTH);
  ...
  private static int validateLoadIndex(int loadIndex) {
    if (loadIndex <= 0) {
      throw new IllegalArgumentException(
          "L'indice di carico dello pneumatico deve essere maggiore di zero.");
    }
  ...
  private static String validateSpeedRating(String speedRating) {
    ...
    if (normalizedSpeedRating.isEmpty()) {
      throw new IllegalArgumentException(
          "Il codice velocità dello pneumatico non può essere vuoto.");
    }

    if (normalizedSpeedRating.length() > MAX_SPEED_RATING_LENGTH) {
      throw new IllegalArgumentException(
          "Il codice velocità dello pneumatico non può superare "
    ...
    if (!normalizedSpeedRating.matches("[A-Z0-9]+")) {
      throw new IllegalArgumentException(
          "Il codice velocità dello pneumatico può contenere solo lettere e numeri.");
    }
