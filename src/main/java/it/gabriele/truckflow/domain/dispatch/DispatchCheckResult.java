...
public final class DispatchCheckResult {
    ...
    private DispatchCheckResult(DispatchCheckType type, DispatchReadinessStatus status, String message) {
        if (type == null) {
    ...
    private static String validateMessage(String message) {
        ...
        if (normalized.length() > MAX_MESSAGE_LENGTH) {
            throw new IllegalArgumentException("Il messaggio controllo dispatch non può superare " + MAX_MESSAGE_LENGTH + " caratteri.");
        }
        return normalized;
    }

    public DispatchCheckType getType() { return type; }
