...
public final class DispatchPlan {
  ...
  private DispatchPlan(
      ...
      List<DispatchAssignmentCandidate> candidates,
      Notes notes) {
  ...
  private static String validateCode(String code) {
    ...
    if (normalized.length() > MAX_CODE_LENGTH) {
      throw new IllegalArgumentException(
          "Il codice piano dispatch non può superare " + MAX_CODE_LENGTH + " caratteri.");
    }
    if (!normalized.matches("[A-Z0-9_-]+")) {
      throw new IllegalArgumentException(
          "Il codice piano dispatch può contenere solo lettere, numeri, trattini e underscore.");
    }
    return normalized;
  }

  private static List<DispatchAssignmentCandidate> validateCandidates(
      List<DispatchAssignmentCandidate> candidates) {
    if (candidates == null) {
    ...
    long uniqueCodes =
        candidates.stream().map(DispatchAssignmentCandidate::getCandidateCode).distinct().count();
  ...
  public List<DispatchAssignmentCandidate> getCandidates() {
    return candidates;
  }
