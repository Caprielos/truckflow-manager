...
/**
 * Candidato di assegnazione per una missione: autista, mezzo/convoglio, parcheggio pronto e controllo margine.
 */
public final class DispatchAssignmentCandidate {
    ...
    private DispatchAssignmentCandidate(
        ...
        this.candidateCode = validateCode(candidateCode, "Il codice candidato dispatch è obbligatorio.");
        ...
        this.vehicleFleetNumber = validateCode(vehicleFleetNumber, "Il numero flotta mezzo dispatch è obbligatorio.");
    ...
    public static DispatchAssignmentCandidate of(
        ...
        return new DispatchAssignmentCandidate(candidateCode, missionNumber, driverId, vehicleFleetNumber, trailerFleetNumber,
                parkedResourceCode, estimatedRevenue, estimatedCost, checks, notes);
    }
    ...
    private static List<DispatchCheckResult> validateChecks(List<DispatchCheckResult> checks) {
        ...
        if (checks.stream().anyMatch(Objects::isNull)) {
            throw new IllegalArgumentException("I controlli candidato dispatch non possono contenere null.");
        }
        long uniqueTypes = checks.stream().map(DispatchCheckResult::getType).distinct().count();
        if (uniqueTypes != checks.size()) {
            throw new IllegalArgumentException("Il candidato dispatch non può avere controlli duplicati dello stesso tipo.");
        }
    ...
    public String getVehicleFleetNumber() { return vehicleFleetNumber; }
    public String getTrailerFleetNumber() { return trailerFleetNumber; }
    public String getParkedResourceCode() { return parkedResourceCode; }
    public Money getEstimatedRevenue() { return estimatedRevenue; }
    public Money getEstimatedCost() { return estimatedCost; }
    public List<DispatchCheckResult> getChecks() { return checks; }
    ...
    public boolean isParkedCombinationReady() {
        return !parkedResourceCode.isEmpty()
                && checks.stream().anyMatch(check -> check.getType() == DispatchCheckType.PARKING_READY
