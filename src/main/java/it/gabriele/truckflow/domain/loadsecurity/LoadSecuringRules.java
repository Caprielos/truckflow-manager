...
/**
 * Regole minime di fissaggio carico.
 * Non sostituisce un calcolo ingegneristico completo, ma impedisce missioni palesemente incomplete.
 */
public final class LoadSecuringRules {
    ...
    public static boolean hasMinimumEquipmentForCargo(
        ...
        if (cargoCategory.isPalletized() || cargoCategory == CargoCategory.GENERAL || cargoCategory == CargoCategory.FOOD) {
            return checklist.hasAtLeast(LoadSecuringEquipmentType.RATCHET_STRAP, estimateMinimumStraps(cargoWeight))
        ...
        if (cargoCategory.requiresTemperatureControl()) {
            return checklist.hasAtLeast(LoadSecuringEquipmentType.LOAD_BAR, 2)
                    || checklist.hasAtLeast(LoadSecuringEquipmentType.RATCHET_STRAP, estimateMinimumStraps(cargoWeight));
        }

        if (cargoCategory.isOversized() || cargoCategory == CargoCategory.MACHINERY || cargoCategory == CargoCategory.VEHICLES) {
            return checklist.hasAtLeast(LoadSecuringEquipmentType.RATCHET_STRAP, estimateMinimumStraps(cargoWeight) + 2)
    ...
    private static void validate(LoadSecuringChecklist checklist, CargoCategory cargoCategory, Weight cargoWeight) {
        if (checklist == null) {
