 ...
 * Registra chi ha fatto cosa, quando e su quale aggregato.
 */
public final class AuditEvent {
    ...
    private static String validateCode(String code, String nullMessage) {
        ...
        if (normalizedCode.length() > MAX_CODE_LENGTH) {
            throw new IllegalArgumentException("Il codice audit non può superare " + MAX_CODE_LENGTH + " caratteri.");
        }

        if (!normalizedCode.matches("[A-Z0-9_-]+")) {
            throw new IllegalArgumentException("Il codice audit può contenere solo lettere, numeri, trattini e underscore.");
        }
    ...
    public boolean isForAggregate(String aggregateType, String aggregateId) {
        return this.aggregateType.equals(validateCode(aggregateType, "Il tipo aggregato da verificare è obbligatorio."))
                && this.aggregateId.equals(validateCode(aggregateId, "L'id aggregato da verificare è obbligatorio."));
    }
    ...
    public boolean isSameAggregate(AuditEvent other) {
        ...
        return aggregateType.equals(other.aggregateType)
