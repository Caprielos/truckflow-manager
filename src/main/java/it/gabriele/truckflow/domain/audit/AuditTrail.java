...
public final class AuditTrail {
  ...
  private AuditTrail(List<AuditEvent> events) {
    ...
    this.events = events.stream().sorted(Comparator.comparing(AuditEvent::getOccurredAt)).toList();
  }
  ...
  private static void validateSameAggregate(List<AuditEvent> events) {
    ...
    boolean allSameAggregate = events.stream().allMatch(first::isSameAggregate);

    if (!allSameAggregate) {
      throw new IllegalArgumentException(
          "Tutti gli eventi audit devono appartenere allo stesso aggregato.");
    }
  }

  private static void validateUniqueEventIds(List<AuditEvent> events) {
    long uniqueEventIds = events.stream().map(AuditEvent::getEventId).distinct().count();
  ...
  public boolean containsEventId(String eventId) {
    ...
    return events.stream().anyMatch(event -> event.getEventId().equals(normalizedEventId));
  }
  ...
  public boolean hasActionType(AuditActionType actionType) {
    ...
    return events.stream().anyMatch(event -> event.getActionType() == actionType);
  }
  ...
  public List<AuditEvent> getEventsByActionType(AuditActionType actionType) {
    ...
    return events.stream().filter(event -> event.getActionType() == actionType).toList();
  }
