...
public final class Shipment {
  ...
  private Shipment(
      String shipmentNumber, TransportOrder transportOrder, ShipmentStatus status, Notes notes) {
  ...
  public static Shipment fromAcceptedOrder(
      String shipmentNumber, TransportOrder transportOrder, Notes notes) {
    if (transportOrder == null) {
      throw new IllegalArgumentException("L'ordine di trasporto è obbligatorio.");
    }
    ...
    if (transportOrder.getStatus() != TransportOrderStatus.ACCEPTED) {
      throw new IllegalArgumentException(
          "Una spedizione può essere creata solo da un ordine accettato.");
    }
  ...
  private static String validateShipmentNumber(String shipmentNumber) {
    ...
    if (normalizedShipmentNumber.length() > MAX_SHIPMENT_NUMBER_LENGTH) {
      throw new IllegalArgumentException(
          "Il numero spedizione non può superare " + MAX_SHIPMENT_NUMBER_LENGTH + " caratteri.");
    }

    if (!normalizedShipmentNumber.matches("[A-Z0-9_-]+")) {
      throw new IllegalArgumentException(
          "Il numero spedizione può contenere solo lettere, numeri, trattini e underscore.");
    }
