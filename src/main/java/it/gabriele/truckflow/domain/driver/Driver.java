...
/**
 * Rappresenta un autista dell'azienda. L'autista è una risorsa operativa, non un account utente.
 */
public final class Driver {
  ...
  private Driver(
    ...
    if (certificates.stream().anyMatch(Objects::isNull)) {
      throw new IllegalArgumentException(
          "I certificati dell'autista non possono contenere valori nulli.");
    }
  ...
  private static String validateDriverCode(String driverCode) {
    ...
    if (normalizedDriverCode.length() > MAX_DRIVER_CODE_LENGTH) {
      throw new IllegalArgumentException(
          "Il codice autista non può superare " + MAX_DRIVER_CODE_LENGTH + " caratteri.");
    }
  ...
  private static String validateFullName(String fullName) {
    ...
    if (normalizedFullName.length() > MAX_FULL_NAME_LENGTH) {
      throw new IllegalArgumentException(
          "Il nome completo dell'autista non può superare " + MAX_FULL_NAME_LENGTH + " caratteri.");
    }
  ...
  private static <T> Set<T> validateRequiredSet(
      Set<T> values, String nullMessage, String emptyMessage, String nullElementMessage) {
    if (values == null) {
  ...
  private static <T> Set<T> validateOptionalSet(
      Set<T> values, String nullMessage, String nullElementMessage) {
    if (values == null) {
      throw new IllegalArgumentException(nullMessage);
    }
  ...
  public boolean hasProfessionalQualification(
      DriverProfessionalQualification professionalQualification) {
    if (professionalQualification == null) {
      throw new IllegalArgumentException(
          "La qualifica professionale da verificare è obbligatoria.");
    }
  ...
  public boolean hasOperationalQualification(
      DriverOperationalQualification operationalQualification) {
  ...
  public boolean hasValidCertificate(DriverCertificateType certificateType, LocalDate date) {
    ...
    return certificates.stream()
        .anyMatch(
            certificate -> certificate.getType() == certificateType && certificate.isValidOn(date));
  }
  ...
  public boolean hasExpiringCertificateWithin(LocalDate date, int warningDays) {
    ...
    return certificates.stream()
        .anyMatch(certificate -> certificate.expiresWithin(date, warningDays));
  }
