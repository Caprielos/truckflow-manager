...
public final class TransportCompany {
    ...
    private TransportCompany(
        ...
        if (licenses.stream().anyMatch(Objects::isNull)) {
            throw new IllegalArgumentException("Le licenze aziendali non possono contenere valori nulli.");
        }
        if (notes == null) {
    ...
    private static String normalizeCode(String value, String nullMessage) {
        ...
        if (normalized.length() > MAX_CODE_LENGTH) {
            throw new IllegalArgumentException("Il codice azienda non può superare " + MAX_CODE_LENGTH + " caratteri.");
        }
        if (!normalized.matches("[A-Z0-9_-]+")) {
            throw new IllegalArgumentException("Il codice azienda può contenere solo lettere, numeri, trattini e underscore.");
        }
        return normalized;
    }
    ...
    private static String normalizeName(String value) {
        ...
        if (normalized.length() > MAX_NAME_LENGTH) {
            throw new IllegalArgumentException("La ragione sociale non può superare " + MAX_NAME_LENGTH + " caratteri.");
        }
        return normalized;
    }
    ...
    public boolean hasValidLicense(CompanyLicenseType type, LocalDate date) {
        ...
        return licenses.stream().anyMatch(license -> license.getType() == type && license.isValidOn(date));
    }
