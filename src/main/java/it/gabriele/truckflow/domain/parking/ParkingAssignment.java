 ...
 * Può rappresentare anche un convoglio già agganciato e pronto per partire.
 */
public final class ParkingAssignment {
    ...
    private ParkingAssignment(
        ...
        this.assignmentCode = validateCode(assignmentCode, "Il codice assegnazione parcheggio è obbligatorio.");
        this.facilityCode = validateCode(facilityCode, "Il codice struttura parcheggio è obbligatorio.");
        ...
        if (endedAt != null && endedAt.isBefore(startedAt)) {
            throw new IllegalArgumentException("La fine assegnazione parcheggio non può essere precedente all'inizio.");
        }
    }
    ...
    public static ParkingAssignment active(
        ...
        return new ParkingAssignment(assignmentCode, spot.getFacilityCode(), spot.getSpotNumber(), parkedResource, startedAt, null, notes);
    }
    ...
    public static ParkingAssignment closed(
        ...
        return new ParkingAssignment(assignmentCode, facilityCode, spotNumber, parkedResource, startedAt, endedAt, notes);
    }
    ...
    private static String validateCode(String code, String message) {
        ...
        if (normalizedCode.length() > MAX_CODE_LENGTH) {
            throw new IllegalArgumentException("Il codice parcheggio non può superare " + MAX_CODE_LENGTH + " caratteri.");
        }

        if (!normalizedCode.matches("[A-Z0-9_-]+")) {
            throw new IllegalArgumentException("Il codice parcheggio può contenere solo lettere, numeri, trattini e underscore.");
        }
    ...
    public boolean occupiesSpot(String facilityCode, String spotNumber) {
        return this.facilityCode.equals(validateCode(facilityCode, "Il codice struttura da verificare è obbligatorio."))
                && this.spotNumber.equals(validateCode(spotNumber, "Il numero posto da verificare è obbligatorio."));
    }

    public boolean containsResource(String resourceId) {
        return parkedResource.includesResource(resourceId) || parkedResource.getResourceId().equals(validateCode(resourceId, "Il codice risorsa da verificare è obbligatorio."));
    }
    ...
    @Override
    public int hashCode() {
        return Objects.hash(assignmentCode, facilityCode, spotNumber, parkedResource, startedAt, endedAt, notes);
    }
