  ...
  public static boolean isSpotFreeAt(
    ...
    if (assignments.stream().anyMatch(Objects::isNull)) {
      throw new IllegalArgumentException(
          "Le assegnazioni parcheggio non possono contenere elementi null.");
    }
    if (moment == null) {
    ...
    return assignments.stream()
        .noneMatch(
            assignment ->
                assignment.occupiesSpot(facilityCode, spotNumber) && assignment.isActiveAt(moment));
  }

  public static boolean isResourceAlreadyParkedAt(
      String resourceId, LocalDateTime moment, List<ParkingAssignment> assignments) {
    if (assignments == null) {
      throw new IllegalArgumentException("Le assegnazioni parcheggio sono obbligatorie.");
    }
    if (assignments.stream().anyMatch(Objects::isNull)) {
    ...
    return assignments.stream()
        .anyMatch(
            assignment -> assignment.containsResource(resourceId) && assignment.isActiveAt(moment));
  }
}
