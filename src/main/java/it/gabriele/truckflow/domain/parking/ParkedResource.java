...
public final class ParkedResource {
  ...
  private ParkedResource(
    ...
    this.resourceId =
        validateIdentifier(resourceId, "Il codice risorsa parcheggiata è obbligatorio.");
  ...
  public static ParkedResource rigidTruck(
      String vehicleId, String displayName, double totalLengthMeters) {
    return new ParkedResource(
        ParkingResourceType.RIGID_TRUCK,
        vehicleId,
        displayName,
        List.of(vehicleId),
        totalLengthMeters,
  ...
  public static ParkedResource tractorUnit(
      String vehicleId, String displayName, double totalLengthMeters) {
    return new ParkedResource(
        ParkingResourceType.TRACTOR_UNIT,
        vehicleId,
        displayName,
        List.of(vehicleId),
        totalLengthMeters,
  ...
  public static ParkedResource trailer(
      String trailerId, String displayName, double totalLengthMeters) {
    return new ParkedResource(
        ParkingResourceType.TRAILER,
        trailerId,
        displayName,
        List.of(trailerId),
        totalLengthMeters,
  ...
  public static ParkedResource semiTrailer(
      String trailerId, String displayName, double totalLengthMeters) {
    return new ParkedResource(
        ParkingResourceType.SEMI_TRAILER,
        trailerId,
        displayName,
        List.of(trailerId),
        totalLengthMeters,
  ...
  public static ParkedResource articulatedVehicle(
    ...
    return new ParkedResource(
        ...
        List.of(
            validateIdentifier(
                tractorUnitId, "Il codice trattore del complesso veicolare è obbligatorio."),
            validateIdentifier(
                semiTrailerId, "Il codice semirimorchio del complesso veicolare è obbligatorio.")),
        totalLengthMeters,
        readyForMission);
  }
  ...
  public static ParkedResource equipment(
      String equipmentId, String displayName, double totalLengthMeters) {
    return new ParkedResource(
        ParkingResourceType.EQUIPMENT,
        equipmentId,
        displayName,
        List.of(equipmentId),
        totalLengthMeters,
  ...
  private static String validateIdentifier(String value, String message) {
    ...
    if (normalizedValue.length() > MAX_ID_LENGTH) {
      throw new IllegalArgumentException(
          "Il codice risorsa parcheggiata non può superare " + MAX_ID_LENGTH + " caratteri.");
    }

    if (!normalizedValue.matches("[A-Z0-9_-]+")) {
      throw new IllegalArgumentException(
          "Il codice risorsa parcheggiata può contenere solo lettere, numeri, trattini e"
              + " underscore.");
    }
  ...
  private static String validateDisplayName(String displayName) {
    ...
    if (normalizedDisplayName.isEmpty()) {
      throw new IllegalArgumentException(
          "La descrizione risorsa parcheggiata non può essere vuota.");
    }

    if (normalizedDisplayName.length() > MAX_DISPLAY_NAME_LENGTH) {
      throw new IllegalArgumentException(
          "La descrizione risorsa parcheggiata non può superare "
  ...
  private static List<String> validateComponentResourceIds(List<String> componentResourceIds) {
    if (componentResourceIds == null || componentResourceIds.isEmpty()) {
      throw new IllegalArgumentException(
          "I componenti della risorsa parcheggiata sono obbligatori.");
    }
    ...
    for (String componentResourceId : componentResourceIds) {
      normalizedIds.add(
          validateIdentifier(
              componentResourceId, "Il codice componente parcheggiato è obbligatorio."));
    }
    return normalizedIds;
  }

  private static double validateLength(double totalLengthMeters) {
    if (Double.isNaN(totalLengthMeters)
        || Double.isInfinite(totalLengthMeters)
        || totalLengthMeters <= 0) {
      throw new IllegalArgumentException(
          "La lunghezza totale risorsa parcheggiata deve essere positiva.");
    }
  ...
  private void validateCombinationConsistency() {
    if (type.isCombination() && componentResourceIds.size() < 2) {
      throw new IllegalArgumentException(
          "Un convoglio parcheggiato deve contenere almeno motrice e unità rimorchiata.");
    }
  ...
  public boolean includesResource(String componentResourceId) {
    String normalizedId =
        validateIdentifier(componentResourceId, "Il codice componente da cercare è obbligatorio.");
  ...
  public int hashCode() {
    return Objects.hash(
        type, resourceId, displayName, componentResourceIds, totalLengthMeters, readyForMission);
  }
