...
public final class InventoryItem {
    ...
    private InventoryItem(
        ...
        this.description = validateText(description, "La descrizione articolo è obbligatoria.", MAX_DESCRIPTION_LENGTH);
        this.unitOfMeasure = validateText(unitOfMeasure, "L'unità di misura articolo è obbligatoria.", MAX_UNIT_LENGTH).toUpperCase();
        if (unitCost == null) {
            throw new IllegalArgumentException("Il costo unitario articolo è obbligatorio.");
        }
        if (minimumStockQuantity < 0 || Double.isNaN(minimumStockQuantity) || Double.isInfinite(minimumStockQuantity)) {
            throw new IllegalArgumentException("La scorta minima deve essere un numero valido non negativo.");
        }
        if (notes == null) {
    ...
    public static InventoryItem of(
        ...
        return new InventoryItem(itemCode, type, description, unitOfMeasure, unitCost, minimumStockQuantity, notes);
    }
    ...
    public double getMinimumStockQuantity() { return minimumStockQuantity; }
    ...
    public Money calculateStockValue(double quantity) {
        if (quantity < 0 || Double.isNaN(quantity) || Double.isInfinite(quantity)) {
            throw new IllegalArgumentException("La quantità da valorizzare deve essere valida e non negativa.");
        }
        return Money.of(unitCost.getAmount().multiply(BigDecimal.valueOf(quantity)), unitCost.getCurrency());
    }
    ...
    @Override
    public int hashCode() {
        return Objects.hash(itemCode, type, description, unitOfMeasure, unitCost, minimumStockQuantity, notes);
    }
}
