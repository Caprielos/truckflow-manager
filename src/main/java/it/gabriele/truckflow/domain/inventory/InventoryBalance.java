...
public final class InventoryBalance {
  ...
  private InventoryBalance(
      InventoryItem item, WarehouseLocation location, double availableQuantity) {
    if (item == null) {
  ...
  public static InventoryBalance of(
      InventoryItem item, WarehouseLocation location, double availableQuantity) {
    return new InventoryBalance(item, location, availableQuantity);
  }

  public static InventoryBalance fromMovements(
      InventoryItem item, WarehouseLocation location, List<InventoryStockMovement> movements) {
    if (movements == null) {
  ...
  public boolean canReserve(double requestedQuantity) {
    if (requestedQuantity <= 0
        || Double.isNaN(requestedQuantity)
        || Double.isInfinite(requestedQuantity)) {
