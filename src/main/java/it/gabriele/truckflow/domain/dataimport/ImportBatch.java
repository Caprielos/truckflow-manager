...
public final class ImportBatch {
    ...
    private static String validateCode(String code) {
        ...
        if (normalized.length() > MAX_CODE_LENGTH) {
            throw new IllegalArgumentException("Il codice lotto import non può superare " + MAX_CODE_LENGTH + " caratteri.");
        }
        if (!normalized.matches("[A-Z0-9_-]+")) {
            throw new IllegalArgumentException("Il codice lotto import può contenere solo lettere, numeri, trattini e underscore.");
        }
        return normalized;
    }

    private static List<ImportRecord> validateRecords(List<ImportRecord> records, ExternalDataSourceType sourceType) {
        if (records == null) {
        ...
        long uniqueExternalRows = records.stream().map(ImportRecord::getExternalRowId).distinct().count();
        ...
        if (wrongSource) {
            throw new IllegalArgumentException("Tutti i record import devono appartenere alla stessa fonte del lotto.");
        }
    ...
    public long countValidatedRecords() {
        return records.stream().filter(record -> record.getStatus() == ImportRecordStatus.VALIDATED).count();
    }

    public long countRejectedRecords() {
        return records.stream().filter(record -> record.getStatus() == ImportRecordStatus.REJECTED).count();
    }
        ...
        for (ImportRecord record : records) {
            if (record.hasAmount()) {
                ...
                if (!amount.getCurrency().equals(currency)) {
                    throw new IllegalArgumentException("Tutti gli importi del lotto devono avere la valuta richiesta.");
                }
