...
public final class TransportOrder {
    ...
    private TransportOrder(
        ...
        if (pickupFacility.equals(deliveryFacility)) {
            throw new IllegalArgumentException("La struttura di ritiro e consegna non possono essere uguali.");
        }
    ...
    private static String validateOrderNumber(String orderNumber) {
        ...
        if (normalizedOrderNumber.length() > MAX_ORDER_NUMBER_LENGTH) {
            throw new IllegalArgumentException("Il numero ordine non può superare " + MAX_ORDER_NUMBER_LENGTH + " caratteri.");
        }

        if (!normalizedOrderNumber.matches("[A-Z0-9_-]+")) {
            throw new IllegalArgumentException("Il numero ordine può contenere solo lettere, numeri, trattini e underscore.");
        }
    ...
    public boolean isInternational() {
        return !pickupFacility.getLocation()
                .getAddress()
                .getCountryCode()
    ...
    public String formatSingleLine() {
        return orderNumber + " - " + customerAccount.getCustomerCode() + " - " + serviceType + " - " + status;
    }
