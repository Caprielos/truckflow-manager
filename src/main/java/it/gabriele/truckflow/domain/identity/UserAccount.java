...
/**
 * Account applicativo.
 * È separato da Driver, Customer e contatti cliente.
 */
public final class UserAccount {
    ...
    private static String validateAccountId(String accountId) {
        ...
        if (normalizedAccountId.length() > MAX_ACCOUNT_ID_LENGTH) {
            throw new IllegalArgumentException("L'id account non può superare "
                    + MAX_ACCOUNT_ID_LENGTH + " caratteri.");
        }

        if (!normalizedAccountId.matches("[A-Z0-9_-]+")) {
            throw new IllegalArgumentException("L'id account può contenere solo lettere, numeri, trattini e underscore.");
        }
    ...
    private static String validateEmail(String email) {
        ...
        if (normalizedEmail.length() > MAX_EMAIL_LENGTH) {
            throw new IllegalArgumentException("L'email account non può superare "
                    + MAX_EMAIL_LENGTH + " caratteri.");
        }
    ...
    private static String validateDisplayName(String displayName) {
        ...
        if (normalizedDisplayName.length() > MAX_DISPLAY_NAME_LENGTH) {
            throw new IllegalArgumentException("Il nome visualizzato account non può superare "
                    + MAX_DISPLAY_NAME_LENGTH + " caratteri.");
        }
    ...
    public boolean hasAnyRole(Set<UserRole> rolesToCheck) {
        ...
        if (rolesToCheck.stream().anyMatch(Objects::isNull)) {
            throw new IllegalArgumentException("I ruoli da verificare non possono contenere valori nulli.");
        }
    ...
    public boolean hasAnyPermission(Set<UserPermission> permissionsToCheck) {
        ...
        if (permissionsToCheck.stream().anyMatch(Objects::isNull)) {
            throw new IllegalArgumentException("I permessi da verificare non possono contenere valori nulli.");
        }
