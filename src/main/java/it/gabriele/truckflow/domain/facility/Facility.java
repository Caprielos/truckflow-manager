...
/**
 * Rappresenta una struttura fisica usata nelle operazioni logistiche.
 * Esempio: magazzino, deposito, terminal, punto cliente o fornitore.
 */
public final class Facility {
    ...
    private static String validateCode(String code) {
        ...
        if (normalizedCode.length() > MAX_CODE_LENGTH) {
            throw new IllegalArgumentException("Il codice della struttura non può superare " + MAX_CODE_LENGTH + " caratteri.");
        }

        if (!normalizedCode.matches("[A-Z0-9_-]+")) {
            throw new IllegalArgumentException("Il codice della struttura può contenere solo lettere, numeri, trattini e underscore.");
        }
