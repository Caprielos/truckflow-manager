...
/**
 * Rappresenta una voce di costo legata a una struttura aziendale.
 * Esempi: affitto mensile, tassa annua, utenze, videosorveglianza, manutenzione piazzale.
 */
public final class FacilityCostLine {
    ...
    public static FacilityCostLine monthly(
        ...
        return new FacilityCostLine(code, type, description, amount, FacilityCostFrequency.MONTHLY, null, notes);
    }
    ...
    public static FacilityCostLine yearly(
        ...
        return new FacilityCostLine(code, type, description, amount, FacilityCostFrequency.YEARLY, null, notes);
    }
    ...
    public static FacilityCostLine oneTime(
        ...
        return new FacilityCostLine(code, type, description, amount, FacilityCostFrequency.ONE_TIME, null, notes);
    }
    ...
    private static String validateCode(String code) {
        ...
        if (normalizedCode.length() > MAX_CODE_LENGTH) {
            throw new IllegalArgumentException("Il codice costo struttura non può superare " + MAX_CODE_LENGTH + " caratteri.");
        }

        if (!normalizedCode.matches("[A-Z0-9_-]+")) {
            throw new IllegalArgumentException("Il codice costo struttura può contenere solo lettere, numeri, trattini e underscore.");
        }
    ...
    private static String validateDescription(String description) {
        ...
        if (normalizedDescription.length() > MAX_DESCRIPTION_LENGTH) {
            throw new IllegalArgumentException("La descrizione costo struttura non può superare " + MAX_DESCRIPTION_LENGTH + " caratteri.");
        }
    ...
    public Money monthlyEquivalent() {
        return switch (frequency) {
            case MONTHLY -> amount;
            case YEARLY -> Money.of(amount.getAmount().divide(BigDecimal.valueOf(12), 2, RoundingMode.HALF_UP), amount.getCurrency());
