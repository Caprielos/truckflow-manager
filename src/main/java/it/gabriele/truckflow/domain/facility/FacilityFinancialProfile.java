...
/**
 * Profilo economico di una struttura fisica.
 * Permette di distinguere una sede di proprietà da un deposito in affitto.
 */
public final class FacilityFinancialProfile {
    ...
    public static FacilityFinancialProfile owned(
        ...
        if (purchasePrice == null) {
            throw new IllegalArgumentException("Il prezzo acquisto struttura di proprietà è obbligatorio.");
        }
        return new FacilityFinancialProfile(facilityCode, FacilityOwnershipType.OWNED, purchasePrice, null, null, costLines, notes);
    }
    ...
    public static FacilityFinancialProfile rented(
        ...
        return new FacilityFinancialProfile(facilityCode, FacilityOwnershipType.RENTED, null, monthlyRent, depositAmount, costLines, notes);
    }
    ...
    public static FacilityFinancialProfile leased(
        ...
        return new FacilityFinancialProfile(facilityCode, FacilityOwnershipType.LEASED, null, monthlyLease, depositAmount, costLines, notes);
    }
    ...
    public static FacilityFinancialProfile thirdPartyYard(
        ...
        return new FacilityFinancialProfile(facilityCode, FacilityOwnershipType.THIRD_PARTY_YARD, null, monthlyFee, depositAmount, costLines, notes);
    }
    ...
    private static String validateFacilityCode(String facilityCode) {
        ...
        if (normalizedCode.length() > MAX_FACILITY_CODE_LENGTH) {
            throw new IllegalArgumentException("Il codice struttura non può superare " + MAX_FACILITY_CODE_LENGTH + " caratteri.");
        }

        if (!normalizedCode.matches("[A-Z0-9_-]+")) {
            throw new IllegalArgumentException("Il codice struttura può contenere solo lettere, numeri, trattini e underscore.");
        }
    ...
    private static List<FacilityCostLine> validateCostLines(List<FacilityCostLine> costLines) {
        ...
        if (costLines.stream().anyMatch(Objects::isNull)) {
            throw new IllegalArgumentException("Le voci costo struttura non possono contenere elementi null.");
        }
    ...
    private void validateEconomicConsistency() {
        if (ownershipType == FacilityOwnershipType.OWNED && monthlyRent != null) {
            throw new IllegalArgumentException("Una struttura di proprietà non deve avere canone mensile di affitto.");
        }

        if (ownershipType.requiresRecurringOccupancyPayment() && monthlyRent == null) {
            throw new IllegalArgumentException("Una struttura non di proprietà deve avere un canone mensile.");
        }

        if (!ownershipType.isOwnedAsset() && purchasePrice != null) {
            throw new IllegalArgumentException("Il prezzo di acquisto è ammesso solo per strutture di proprietà.");
        }
    ...
    private void validateSameCurrencyWhenPresent() {
        ...
        if (costLines.stream().anyMatch(line -> !line.getAmount().getCurrency().equals(finalReferenceCurrency))) {
            throw new IllegalArgumentException("Gli importi struttura devono avere la stessa valuta.");
        }
    }
    ...
    @Override
    public int hashCode() {
        return Objects.hash(facilityCode, ownershipType, purchasePrice, monthlyRent, depositAmount, costLines, notes);
    }
