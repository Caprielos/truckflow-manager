...
/**
 * Regola di paga configurabile: può essere oraria, giornaliera, a missione,
 * rimborso puro o percentuale sul costo base.
 */
public final class DriverPayRule {
    ...
    private DriverPayRule(
        ...
        if (unit.usesPercentage()) {
            ...
            if (amount.getAmount().signum() == 0) {
                throw new IllegalArgumentException("L'importo della regola paga deve essere maggiore di zero.");
            }
            if (percentage != null) {
    ...
    public static DriverPayRule percentageOfBase(
        ...
        return new DriverPayRule(ruleCode, componentType, DriverPayUnit.PERCENTAGE_OF_BASE, description, null, percentage, notes);
    }
    ...
    public DriverMissionPayLine calculateLine(BigDecimal quantity, Money baseAmount) {
        ...
        if (unit.usesPercentage()) {
            if (baseAmount == null) {
                throw new IllegalArgumentException("Il costo base è obbligatorio per calcolare una regola percentuale.");
            }
            calculatedAmount = Money.of(
                    baseAmount.getAmount().multiply(percentage.toMultiplier()).setScale(2, RoundingMode.HALF_UP),
                    baseAmount.getCurrency()
            );
        } else {
            calculatedAmount = Money.of(amount.getAmount().multiply(quantity).setScale(2, RoundingMode.HALF_UP), amount.getCurrency());
        }

        return DriverMissionPayLine.of(ruleCode, componentType, unit, description, quantity, calculatedAmount, notes);
    }
    ...
    private static String validateCode(String code) {
        ...
        if (normalized.length() > MAX_CODE_LENGTH) {
            throw new IllegalArgumentException("Il codice regola paga non può superare " + MAX_CODE_LENGTH + " caratteri.");
        }
        if (!normalized.matches("[A-Z0-9_-]+")) {
            throw new IllegalArgumentException("Il codice regola paga può contenere solo lettere, numeri, trattini e underscore.");
        }
        return normalized;
    }
    ...
    private static String validateDescription(String description) {
        ...
        if (normalized.length() > MAX_DESCRIPTION_LENGTH) {
            throw new IllegalArgumentException("La descrizione regola paga non può superare " + MAX_DESCRIPTION_LENGTH + " caratteri.");
        }
        return normalized;
    }
