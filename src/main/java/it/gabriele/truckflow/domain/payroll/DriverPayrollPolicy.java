...
/**
 * Politica paga aziendale/contrattuale applicata agli autisti.
 * Le cifre non sono hard-coded nel dominio: vengono configurate per azienda,
 * paese, contratto e periodo di validità.
 */
public final class DriverPayrollPolicy {
    ...
    private DriverPayrollPolicy(
        ...
        if (validTo != null && validTo.isBefore(validFrom)) {
            throw new IllegalArgumentException("La data fine validità politica paga non può precedere l'inizio.");
        }
        if (rules == null) {
    ...
    private static String validateDescription(String description) {
        ...
        if (normalized.length() > MAX_DESCRIPTION_LENGTH) {
            throw new IllegalArgumentException("La descrizione politica paga non può superare " + MAX_DESCRIPTION_LENGTH + " caratteri.");
        }
        return normalized;
    }
