...
/**
 * Cedolino/costo autista calcolato per una missione. Include sia importi pagati/rimborsati
 * all'autista sia costi aziendali collegati.
 */
public final class DriverMissionPayroll {
  ...
  private DriverMissionPayroll(
    ...
    this.payrollPolicyCode =
        validateCode(payrollPolicyCode, "Il codice politica paga è obbligatorio.");
    if (payLines == null) {
    ...
    if (uniqueCodes != payLines.size()) {
      throw new IllegalArgumentException(
          "Le righe payroll missione non possono avere codici duplicati.");
    }
  ...
  public static DriverMissionPayroll of(
      ...
      List<DriverMissionPayLine> payLines,
      Notes notes) {
    return new DriverMissionPayroll(
        payrollCode, missionNumber, driverCode, payrollPolicyCode, payLines, notes);
  }
  ...
  public Money calculateAllowancesAndReimbursements() {
    List<DriverMissionPayLine> selected =
        payLines.stream().filter(DriverMissionPayLine::isAllowanceOrReimbursement).toList();
    return sumOrZero(selected);
  }

  public Money calculateEmployerCharges() {
    List<DriverMissionPayLine> selected =
        payLines.stream().filter(DriverMissionPayLine::isEmployerCost).toList();
    return sumOrZero(selected);
  }
