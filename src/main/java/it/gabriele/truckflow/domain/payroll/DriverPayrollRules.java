...
/**
 * Regole di calcolo del costo autista. Non decide importi per legge: applica una politica
 * configurata e attiva solo le voci coerenti con missione, patenti, ADR, merce, allestimento e
 * convoglio.
 */
public final class DriverPayrollRules {
  ...
  public static DriverMissionPayroll calculateMissionPayroll(
      String payrollCode, DriverMissionWorkReport report, DriverPayrollPolicy policy, Notes notes) {
    if (report == null) {
    ...
    addAmountLine(
        lines, policy, DriverPayComponentType.BASE_DRIVING_TIME, report.getDrivingHours(), null);
    addAmountLine(
        lines, policy, DriverPayComponentType.BASE_OTHER_WORK, report.getOtherWorkHours(), null);
    addAmountLine(
        lines, policy, DriverPayComponentType.WAITING_TIME, report.getWaitingHours(), null);
    addAmountLine(
        lines,
        policy,
        DriverPayComponentType.LOADING_UNLOADING,
        report.getLoadingUnloadingHours(),
        null);
    addAmountLine(lines, policy, DriverPayComponentType.OVERTIME, report.getOvertimeHours(), null);
    addAmountLine(
        lines, policy, DriverPayComponentType.NIGHT_WORK, report.getNightWorkHours(), null);
    addAmountLine(
        lines, policy, DriverPayComponentType.SUNDAY_WORK, report.getSundayWorkHours(), null);
    addAmountLine(
        lines, policy, DriverPayComponentType.HOLIDAY_WORK, report.getHolidayWorkHours(), null);

    addAmountLine(
        lines,
        policy,
        DriverPayComponentType.DOMESTIC_DAILY_ALLOWANCE,
        BigDecimal.valueOf(report.getDomesticAllowanceDays()),
        null);
    addAmountLine(
        lines,
        policy,
        DriverPayComponentType.INTERNATIONAL_DAILY_ALLOWANCE,
        BigDecimal.valueOf(report.getInternationalAllowanceDays()),
        null);
    addAmountLine(
        lines,
        policy,
        DriverPayComponentType.OVERNIGHT_ALLOWANCE,
        BigDecimal.valueOf(report.getOvernightDays()),
        null);
    ...
    if (report.transportsAdrCargo()
        && report.driverHasAdrCertificate(DriverAdrCertificateType.ADR_BASIC)) {
      addAmountLine(
          lines, policy, DriverPayComponentType.ADR_BASIC_PREMIUM, report.getDrivingHours(), null);
    }
    if (report.transportsLiquidOrTankCargo()
        && report.driverHasAdrCertificate(DriverAdrCertificateType.ADR_TANK)) {
      addAmountLine(
          lines, policy, DriverPayComponentType.ADR_TANK_PREMIUM, report.getDrivingHours(), null);
    }
    if (report.driverHasAdrCertificate(DriverAdrCertificateType.ADR_CLASS_1_EXPLOSIVES)) {
      addAmountLine(
          lines,
          policy,
          DriverPayComponentType.ADR_CLASS_1_PREMIUM,
          report.getDrivingHours(),
          null);
    }
    if (report.driverHasAdrCertificate(DriverAdrCertificateType.ADR_CLASS_7_RADIOACTIVE)) {
      addAmountLine(
          lines,
          policy,
          DriverPayComponentType.ADR_CLASS_7_PREMIUM,
          report.getDrivingHours(),
          null);
    }
    ...
    if (report.transportsWaste()
        && report.driverHasOperationalQualification(
            DriverOperationalQualification.WASTE_TRANSPORT)) {
      addAmountLine(
          lines,
          policy,
    ...
    if (report.transportsTemperatureControlledCargo()
      ...
      addAmountLine(
          lines,
          policy,
          DriverPayComponentType.REFRIGERATED_TRANSPORT_PREMIUM,
          report.getDrivingHours(),
          null);
    }
    if (report.transportsPharmaceuticals()) {
      addAmountLine(
          lines,
          policy,
          DriverPayComponentType.PHARMA_TRANSPORT_PREMIUM,
          report.getDrivingHours(),
          null);
    }
    if (report.transportsFoodGradeCargo()) {
      addAmountLine(
          lines,
          policy,
          DriverPayComponentType.FOOD_GRADE_TRANSPORT_PREMIUM,
          report.getDrivingHours(),
          null);
    }
    if (report.transportsLivestock()
        && report.driverHasOperationalQualification(
            DriverOperationalQualification.LIVE_ANIMAL_TRANSPORT)) {
      addAmountLine(
          lines,
          policy,
          DriverPayComponentType.LIVESTOCK_TRANSPORT_PREMIUM,
          report.getDrivingHours(),
          null);
    }
    if (report.transportsHighValueCargo()
        && report.driverHasOperationalQualification(
            DriverOperationalQualification.HIGH_VALUE_CARGO)) {
      addAmountLine(
          lines,
          policy,
          DriverPayComponentType.HIGH_VALUE_CARGO_PREMIUM,
          report.getDrivingHours(),
          null);
    }
    if (report.transportsOversizedCargo()
        && report.driverHasOperationalQualification(
            DriverOperationalQualification.OVERSIZED_CARGO)) {
      addAmountLine(
          lines,
          policy,
          DriverPayComponentType.OVERSIZED_CARGO_PREMIUM,
          report.getDrivingHours(),
          null);
    }
    if (report.transportsBulkCargo()
        && report.driverHasOperationalQualification(
            DriverOperationalQualification.BULK_TRANSPORT)) {
      addAmountLine(
          lines,
          policy,
          DriverPayComponentType.BULK_TRANSPORT_PREMIUM,
          report.getDrivingHours(),
          null);
    }
    if (report.transportsLiquidOrTankCargo()) {
      addAmountLine(
          lines,
          policy,
          DriverPayComponentType.LIQUID_TANKER_PREMIUM,
          report.getDrivingHours(),
          null);
    }
    ...
    if (report.usesArticulatedVehicle()) {
      addAmountLine(
          lines, policy, DriverPayComponentType.ARTICULATED_VEHICLE_PREMIUM, BigDecimal.ONE, null);
      addAmountLine(
          lines, policy, DriverPayComponentType.SEMI_TRAILER_PREMIUM, BigDecimal.ONE, null);
    }
    if (report.usesTruckAndTrailer()) {
      addAmountLine(
          lines, policy, DriverPayComponentType.TRUCK_AND_TRAILER_PREMIUM, BigDecimal.ONE, null);
      addAmountLine(
          lines, policy, DriverPayComponentType.DRAWBAR_TRAILER_PREMIUM, BigDecimal.ONE, null);
    }
    ...
    if (report.usesTipperBody()) {
      addAmountLine(
          lines, policy, DriverPayComponentType.TIPPER_BODY_PREMIUM, BigDecimal.ONE, null);
    }
    if (report.usesLowLoaderBody()) {
      addAmountLine(
          lines, policy, DriverPayComponentType.LOW_LOADER_BODY_PREMIUM, BigDecimal.ONE, null);
    }
    if (report.usesRefrigeratedBody()) {
      addAmountLine(
          lines, policy, DriverPayComponentType.REFRIGERATED_BODY_PREMIUM, BigDecimal.ONE, null);
    }
    if (report.usesCrane()
        && report.driverHasOperationalQualification(
            DriverOperationalQualification.TRUCK_MOUNTED_CRANE)) {
      addAmountLine(
          lines,
          policy,
          DriverPayComponentType.CRANE_OPERATION_PREMIUM,
          report.getLoadingUnloadingHours(),
          null);
    }
    if (report.usesTailLift()) {
      addAmountLine(
          lines,
          policy,
          DriverPayComponentType.TAIL_LIFT_OPERATION_PREMIUM,
          report.getLoadingUnloadingHours(),
          null);
    }
    ...
    addAmountLine(
        lines,
        policy,
        DriverPayComponentType.HOTEL_REIMBURSEMENT,
        BigDecimal.valueOf(report.getOvernightDays()),
        null);
    addAmountLine(
        lines, policy, DriverPayComponentType.PARKING_REIMBURSEMENT, BigDecimal.ONE, null);
    ...
    if (lines.isEmpty()) {
      throw new IllegalArgumentException(
          "La politica paga non ha prodotto righe: configurare almeno una regola applicabile.");
    }
  ...
  private static Money calculateBaseLabourCost(List<DriverMissionPayLine> lines) {
    List<DriverMissionPayLine> baseLines =
        lines.stream()
