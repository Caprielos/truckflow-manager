...
public final class SystemConfiguration {
  ...
  public static SystemConfiguration activeForScope(
      ...
      Notes notes) {
    return new SystemConfiguration(
        configurationKey, category, scope, scopeReference, value, description, true, notes);
  }
  ...
  public static SystemConfiguration inactiveForScope(
      ...
      Notes notes) {
    return new SystemConfiguration(
        configurationKey, category, scope, scopeReference, value, description, false, notes);
  }
  ...
  private static String validateConfigurationKey(String configurationKey) {
    ...
    if (normalizedKey.length() > MAX_KEY_LENGTH) {
      throw new IllegalArgumentException(
          "La chiave configurazione non può superare " + MAX_KEY_LENGTH + " caratteri.");
    }

    if (!normalizedKey.matches("[A-Z0-9_.-]+")) {
      throw new IllegalArgumentException(
          "La chiave configurazione può contenere solo lettere, numeri, punti, trattini e"
              + " underscore.");
    }
  ...
  private static String validateScopeReference(ConfigurationScope scope, String scopeReference) {
    if (scope.isGlobal()) {
      return "GLOBAL";
    }
    ...
    if (normalizedScopeReference.isEmpty()) {
      throw new IllegalArgumentException(
          "Il riferimento ambito configurazione non può essere vuoto.");
    }

    if (normalizedScopeReference.length() > MAX_SCOPE_REFERENCE_LENGTH) {
      throw new IllegalArgumentException(
          "Il riferimento ambito configurazione non può superare "
    ...
    if (!normalizedScopeReference.matches("[A-Z0-9_.-]+")) {
      throw new IllegalArgumentException(
          "Il riferimento ambito configurazione può contenere solo lettere, numeri, punti, trattini"
              + " e underscore.");
    }
  ...
  private static String validateDescription(String description) {
    ...
    if (normalizedDescription.length() > MAX_DESCRIPTION_LENGTH) {
      throw new IllegalArgumentException(
          "La descrizione configurazione non può superare "
  ...
  private SystemConfiguration copyWith(
      boolean currentActive, ConfigurationValue newValue, boolean newActive) {
    return new SystemConfiguration(
        configurationKey, category, scope, scopeReference, newValue, description, newActive, notes);
  }
  ...
  public boolean isForScope(ConfigurationScope scope, String scopeReference) {
    if (scope == null) {
  ...
  public int hashCode() {
    return Objects.hash(
        configurationKey, category, scope, scopeReference, value, description, active, notes);
  }
