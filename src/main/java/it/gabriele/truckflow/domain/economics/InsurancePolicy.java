...
public final class InsurancePolicy {
  ...
  private InsurancePolicy(
    ...
    this.insuredAssetCode =
        validateCode(insuredAssetCode, "Il codice bene assicurato è obbligatorio.");
  ...
  private static String validateProviderName(String providerName) {
    ...
    if (normalized.length() > MAX_PROVIDER_LENGTH) {
      throw new IllegalArgumentException(
          "Il nome compagnia assicurativa non può superare " + MAX_PROVIDER_LENGTH + " caratteri.");
    }
    return normalized;
  }
