...
/**
 * Bene aziendale acquistato: camion, trattore, rimorchio, allestimento, frigo, attrezzatura, gomme, telematica.
 * Conserva il costo storico e permette di allocare l'ammortamento nei periodi o nelle missioni.
 */
public final class FleetAssetPurchase {
    ...
    private FleetAssetPurchase(
        ...
        this.supplierInvoiceNumber = validateCode(supplierInvoiceNumber, "Il numero fattura fornitore è obbligatorio.");
        ...
        if (residualValue.isGreaterThan(purchasePrice)) {
            throw new IllegalArgumentException("Il valore residuo non può superare il prezzo di acquisto.");
        }
    ...
    public static FleetAssetPurchase of(
        ...
        return new FleetAssetPurchase(assetCode, category, supplierInvoiceNumber, assignedVehicleFleetNumber,
                description, purchaseDate, purchasePrice, residualValue, usefulLifeMonths, notes);
    }
    ...
    public static FleetAssetPurchase vehicle(
        ...
        return of(assetCode, category, supplierInvoiceNumber, assignedVehicleFleetNumber, description,
                purchaseDate, purchasePrice, residualValue, usefulLifeMonths, notes);
    }
    ...
    private static String validateDescription(String description) {
        ...
        if (normalized.length() > MAX_DESCRIPTION_LENGTH) {
            throw new IllegalArgumentException("La descrizione bene aziendale non può superare "
    ...
    public Money calculateMonthlyDepreciation() {
        BigDecimal monthly = calculateDepreciableValue().getAmount()
    ...
    public Money calculateDepreciationForMonths(int months) {
        ...
        BigDecimal amount = calculateMonthlyDepreciation().getAmount().multiply(BigDecimal.valueOf(months));
    ...
    public Money calculateDepreciationForDays(long days) {
        ...
        BigDecimal amount = calculateDepreciableValue().getAmount()
    ...
    @Override
    public int hashCode() {
        return Objects.hash(assetCode, category, supplierInvoiceNumber, assignedVehicleFleetNumber, description,
                purchaseDate, purchasePrice, residualValue, usefulLifeMonths, notes);
    }
}
