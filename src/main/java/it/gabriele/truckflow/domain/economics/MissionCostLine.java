...
/**
 * Voce costo collegata a una missione: carburante, pedaggi, autista, gomme, manutenzione,
...
public final class MissionCostLine {
  ...
  private MissionCostLine(
      String lineCode, MissionCostType type, String description, Money amount, Notes notes) {
  ...
  public static MissionCostLine of(
      String lineCode, MissionCostType type, String description, Money amount, Notes notes) {
    return new MissionCostLine(lineCode, type, description, amount, notes);
  }

  public static MissionCostLine fuelFromTransaction(
      String lineCode, FuelTransaction transaction, Notes notes) {
    if (transaction == null) {
    ...
    BigDecimal amount =
        transaction
            .getPricePerLiter()
            .getAmount()
            .multiply(BigDecimal.valueOf(transaction.getLiters()));
    Money total = Money.of(amount, transaction.getPricePerLiter().getCurrency());
    return of(
        lineCode,
        MissionCostType.FUEL,
        "Rifornimento " + transaction.getCardProvider(),
        total,
        notes);
  }

  public static MissionCostLine fuelFromEstimate(
      String lineCode, RouteCostEstimate estimate, Notes notes) {
    validateEstimate(estimate);
    return of(
        lineCode,
        MissionCostType.FUEL,
        "Carburante stimato da " + estimate.getSource(),
        estimate.getEstimatedFuelCost(),
        notes);
  }

  public static MissionCostLine tollsFromEstimate(
      String lineCode, RouteCostEstimate estimate, Notes notes) {
    validateEstimate(estimate);
    return of(
        lineCode,
        MissionCostType.TOLL,
        "Pedaggi stimati da " + estimate.getSource(),
        estimate.getEstimatedTollCost(),
        notes);
  }

  public static MissionCostLine vehicleWearFromEstimate(
      String lineCode, RouteCostEstimate estimate, Notes notes) {
    validateEstimate(estimate);
    return of(
        lineCode,
        MissionCostType.VEHICLE_DEPRECIATION,
        "Usura mezzo stimata da " + estimate.getSource(),
        estimate.getEstimatedVehicleWearCost(),
        notes);
  }

  public static MissionCostLine depreciation(
      String lineCode, FleetAssetPurchase asset, int months, Notes notes) {
    if (asset == null) {
      throw new IllegalArgumentException("Il bene aziendale è obbligatorio.");
    }
    MissionCostType type =
        switch (asset.getCategory()) {
          case SEMI_TRAILER, DRAWBAR_TRAILER, CENTER_AXLE_TRAILER ->
              MissionCostType.TRAILER_DEPRECIATION;
          case BODY_EQUIPMENT, REFRIGERATION_UNIT, LOADING_EQUIPMENT ->
              MissionCostType.BODY_EQUIPMENT_DEPRECIATION;
          default -> MissionCostType.VEHICLE_DEPRECIATION;
        };
    return of(
        lineCode,
        type,
        "Ammortamento " + asset.getAssetCode(),
        asset.calculateDepreciationForMonths(months),
        notes);
  }
  ...
  private static String validateDescription(String description) {
    ...
    if (normalized.length() > MAX_DESCRIPTION_LENGTH) {
      throw new IllegalArgumentException(
          "La descrizione costo missione non può superare "
