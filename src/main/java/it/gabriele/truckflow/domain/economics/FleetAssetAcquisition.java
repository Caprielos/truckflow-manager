...
/**
 * Acquisto reale composto: trattore + semirimorchio + allestimento + frigo + gomme + telematica +
 * pratiche. Tiene distinti imponibile, IVA, lordo pagato e costo contabile effettivo.
 */
public final class FleetAssetAcquisition {
  ...
  private FleetAssetAcquisition(
      ...
      Notes notes) {
    this.acquisitionNumber =
        validateCode(acquisitionNumber, "Il numero acquisto flotta è obbligatorio.");
    this.supplierCode =
        validateCode(supplierCode, "Il codice fornitore acquisto flotta è obbligatorio.");
    this.supplierInvoiceNumber =
        validateCode(supplierInvoiceNumber, "Il numero fattura acquisto flotta è obbligatorio.");
  ...
  public static FleetAssetAcquisition of(
      ...
      List<FleetAssetCostComponent> components,
      Notes notes) {
    return new FleetAssetAcquisition(
        acquisitionNumber, supplierCode, supplierInvoiceNumber, purchaseDate, components, notes);
  }
  ...
  private static List<FleetAssetCostComponent> validateComponents(
      List<FleetAssetCostComponent> components) {
    if (components == null) {
    ...
    if (components.stream().anyMatch(Objects::isNull)) {
      throw new IllegalArgumentException(
          "I componenti acquisto flotta non possono contenere null.");
    }
    long uniqueCodes =
        components.stream().map(FleetAssetCostComponent::getComponentCode).distinct().count();
    if (uniqueCodes != components.size()) {
      throw new IllegalArgumentException(
          "I componenti acquisto flotta non possono avere codici duplicati.");
    }
  ...
  public Money calculateRecoverableVatTotal() {
    return sum(
        components.stream().map(FleetAssetCostComponent::calculateRecoverableVatAmount).toList());
  }
