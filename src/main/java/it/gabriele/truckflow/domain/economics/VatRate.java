...
/**
 * Aliquota IVA usata in una riga economica.
 * Sono presenti factory comode per le aliquote italiane più comuni, ma il valore resta configurabile.
 */
public final class VatRate {
    ...
    private static String validateCode(String code) {
        ...
        if (normalized.length() > MAX_CODE_LENGTH) {
            throw new IllegalArgumentException("Il codice aliquota IVA non può superare " + MAX_CODE_LENGTH + " caratteri.");
        }
        if (!normalized.matches("[A-Z0-9_%-]+")) {
            throw new IllegalArgumentException("Il codice aliquota IVA può contenere solo lettere, numeri, trattini, underscore e percentuale.");
        }
        return normalized;
    }
