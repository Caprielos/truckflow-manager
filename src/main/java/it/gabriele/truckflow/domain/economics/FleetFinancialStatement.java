...
/**
 * Sintesi economica di un periodo: ricavi, costi operativi, acquisti, ammortamenti e risultato
 * cassa.
 */
public final class FleetFinancialStatement {
  ...
  private FleetFinancialStatement(
    ...
    this.statementNumber =
        validateCode(statementNumber, "Il numero prospetto economico è obbligatorio.");
    if (period == null) {
    ...
    if (this.missions.isEmpty()
        && this.supplierInvoices.isEmpty()
        && this.assetPurchases.isEmpty()) {
      throw new IllegalArgumentException(
          "Il prospetto economico deve contenere almeno un movimento.");
    }
    if (notes == null) {
  ...
  public static FleetFinancialStatement of(
      ...
      List<FleetAssetPurchase> assetPurchases,
      Notes notes) {
    return new FleetFinancialStatement(
        statementNumber, period, missions, supplierInvoices, assetPurchases, notes);
  }
  ...
  private static <T> List<T> validateList(List<T> values, String nullMessage) {
    ...
    if (values.stream().anyMatch(Objects::isNull)) {
      throw new IllegalArgumentException(
          "Le liste del prospetto economico non possono contenere null.");
    }
  ...
  /**
   * Risultato di cassa: qui gli acquisti di camion/rimorchi/allestimenti pesano subito come uscita
   * di denaro.
   */
