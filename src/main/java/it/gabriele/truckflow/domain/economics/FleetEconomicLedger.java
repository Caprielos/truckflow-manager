...
/**
 * Libro economico reale di periodo. Confronta ricavi, costi operativi, acquisti, IVA, spese
 * ricorrenti, finanziamenti e cassa.
 */
public final class FleetEconomicLedger {
  ...
  private FleetEconomicLedger(
    ...
    this.supplierInvoices =
        validateList(supplierInvoices, "Le fatture fornitore sono obbligatorie.");
    this.assetAcquisitions =
        validateList(assetAcquisitions, "Gli acquisti flotta sono obbligatori.");
    this.recurringExpenses =
        validateList(recurringExpenses, "Le spese ricorrenti sono obbligatorie.");
    this.financingAgreements =
        validateList(financingAgreements, "I finanziamenti sono obbligatori.");
    if (notes == null) {
  ...
  public static FleetEconomicLedger of(
      ...
      List<FinancingAgreement> financingAgreements,
      Notes notes) {
    return new FleetEconomicLedger(
        ledgerNumber,
        period,
        openingCash,
        customerInvoices,
  ...
  private static <T> List<T> validateList(List<T> values, String nullMessage) {
    ...
    if (values.stream().anyMatch(Objects::isNull)) {
      throw new IllegalArgumentException(
          "Le liste del libro economico non possono contenere null.");
    }
  ...
  public Money calculateCustomerGrossInvoiced() {
    if (!customerInvoices.isEmpty()) {
      return sum(
          customerInvoices.stream().map(CustomerRevenueInvoice::calculateGrossTotal).toList());
    }
  ...
  public Money calculateSupplierInvoiceAccountingCost() {
    return sum(
        supplierInvoices.stream().map(SupplierInvoice::calculateAccountingCostTotal).toList());
  }

  public Money calculateSupplierRecoverableVat() {
    return sum(
        supplierInvoices.stream().map(SupplierInvoice::calculateRecoverableVatTotal).toList());
  }
  ...
  public Money calculateAssetAccountingCost() {
    return sum(
        assetAcquisitions.stream()
            .map(FleetAssetAcquisition::calculateAccountingCostTotal)
            .toList());
  }
  ...
  public Money calculateAssetRecoverableVat() {
    return sum(
        assetAcquisitions.stream()
            .map(FleetAssetAcquisition::calculateRecoverableVatTotal)
            .toList());
  }
  ...
  public Money calculateFinancingCashOutflow() {
    return sum(
        financingAgreements.stream().map(FinancingAgreement::calculateTotalCashOutflow).toList());
  }
  ...
  public FinancialBalance calculateVatPosition() {
    return FinancialBalance.from(calculateSalesVatCollected())
        .subtract(calculateRecoverableVatTotal());
  }
  ...
  /**
   * Risultato di cassa: considera quello che entra/esce davvero dal conto, incluso lordo IVA e
   * acquisti.
   */
