...
public final class MissionEconomics {
    ...
    private static List<MissionRevenueLine> validateRevenueLines(List<MissionRevenueLine> lines) {
        ...
        if (uniqueCodes != lines.size()) {
            throw new IllegalArgumentException("Le righe ricavo missione non possono avere codici duplicati.");
        }
    ...
    private static List<MissionCostLine> validateCostLines(List<MissionCostLine> lines) {
        ...
        if (uniqueCodes != lines.size()) {
            throw new IllegalArgumentException("Le righe costo missione non possono avere codici duplicati.");
        }
        return List.copyOf(lines);
    }

    private static void validateCurrencyCompatibility(List<MissionRevenueLine> revenueLines, List<MissionCostLine> costLines) {
    ...
    public Money calculateVariableCosts() {
        List<MissionCostLine> variableCosts = costLines.stream()
                .filter(MissionCostLine::isVariableOperationalCost)
                .toList();
