...
/**
 * Scomposizione di un importo tra imponibile, IVA, totale lordo e IVA recuperabile.
 * Per gli acquisti aziendali il costo contabile è imponibile + IVA non recuperabile.
 */
public final class VatBreakdown {
    ...
    private VatBreakdown(
        ...
        if (!treatment.generatesVatAmount() && vatAmount.getAmount().signum() > 0) {
            throw new IllegalArgumentException("Un trattamento IVA non imponibile/esente/non soggetto non può avere IVA calcolata.");
        }
    ...
    public static VatBreakdown taxableFromNet(Money netAmount, VatRate vatRate, Percentage recoverableVatPercentage) {
        validateNetAndRate(netAmount, vatRate);
        Money vat = calculateVatFromNet(netAmount, vatRate);
        return new VatBreakdown(netAmount, vatRate, VatTreatment.TAXABLE, vat,
                netAmount.add(vat), recoverableVatPercentage);
    }
    ...
    public static VatBreakdown taxableFromGross(Money grossAmount, VatRate vatRate, Percentage recoverableVatPercentage) {
        if (grossAmount == null) {
            throw new IllegalArgumentException("Il lordo IVA è obbligatorio.");
        }
        if (vatRate == null) {
        ...
        return new VatBreakdown(net, vatRate, VatTreatment.TAXABLE, vat, grossAmount, recoverableVatPercentage);
    }
    ...
    public static VatBreakdown nonDeductibleFromNet(Money netAmount, VatRate vatRate) {
        validateNetAndRate(netAmount, vatRate);
        Money vat = calculateVatFromNet(netAmount, vatRate);
        return new VatBreakdown(netAmount, vatRate, VatTreatment.NOT_DEDUCTIBLE, vat,
                netAmount.add(vat), NOT_RECOVERABLE);
    }
    ...
    private static VatBreakdown withoutVat(Money amount, VatRate vatRate, VatTreatment treatment) {
        ...
        return new VatBreakdown(amount, vatRate, treatment,
                Money.of(BigDecimal.ZERO, amount.getCurrency()), amount, NOT_RECOVERABLE);
    }
    ...
    private static Money calculateVatFromNet(Money netAmount, VatRate vatRate) {
        BigDecimal value = netAmount.getAmount()
                .multiply(vatRate.toMultiplier())
    ...
    public Money calculateRecoverableVatAmount() {
        BigDecimal value = vatAmount.getAmount()
    ...
    @Override
    public int hashCode() {
        return Objects.hash(netAmount, vatRate, treatment, vatAmount, grossAmount, recoverableVatPercentage);
    }
}
