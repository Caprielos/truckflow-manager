...
/**
 * Stima dei costi legati al percorso. Può arrivare da calcolo manuale, modello interno o provider
 * esterno.
 */
public final class RouteCostEstimate {
  ...
  private static String validateEstimateCode(String estimateCode) {
    ...
    if (normalizedEstimateCode.length() > MAX_ESTIMATE_CODE_LENGTH) {
      throw new IllegalArgumentException(
          "Il codice stima costi non può superare " + MAX_ESTIMATE_CODE_LENGTH + " caratteri.");
    }

    if (!normalizedEstimateCode.matches("[A-Z0-9_-]+")) {
      throw new IllegalArgumentException(
          "Il codice stima costi può contenere solo lettere, numeri, trattini e underscore.");
    }
  ...
  private static void validateCurrencyCompatibility(
      Money estimatedFuelCost, Money estimatedTollCost, Money estimatedVehicleWearCost) {
  ...
  public Money calculateEstimatedRouteCost() {
    return estimatedFuelCost.add(estimatedTollCost).add(estimatedVehicleWearCost);
  }
