...
public final class PricingLine {
    ...
    private static String validateLineCode(String lineCode) {
        ...
        if (normalizedLineCode.length() > MAX_LINE_CODE_LENGTH) {
            throw new IllegalArgumentException("Il codice voce prezzo non può superare "
                    + MAX_LINE_CODE_LENGTH + " caratteri.");
        }

        if (!normalizedLineCode.matches("[A-Z0-9_-]+")) {
            throw new IllegalArgumentException("Il codice voce prezzo può contenere solo lettere, numeri, trattini e underscore.");
        }
    ...
    private static String validateDescription(String description) {
        ...
        if (normalizedDescription.length() > MAX_DESCRIPTION_LENGTH) {
            throw new IllegalArgumentException("La descrizione della voce prezzo non può superare "
