...
public final class PriceBreakdown {
  ...
  private PriceBreakdown(String quoteNumber, List<PricingLine> lines, Notes notes) {
  ...
  public static PriceBreakdown of(String quoteNumber, List<PricingLine> lines, Notes notes) {
    return new PriceBreakdown(quoteNumber, lines, notes);
  }

  public static PriceBreakdown of(
      String quoteNumber, PricingLine firstLine, PricingLine... otherLines) {
    if (firstLine == null) {
  ...
  private static String validateQuoteNumber(String quoteNumber) {
    ...
    if (normalizedQuoteNumber.length() > MAX_QUOTE_NUMBER_LENGTH) {
      throw new IllegalArgumentException(
          "Il numero preventivo non può superare " + MAX_QUOTE_NUMBER_LENGTH + " caratteri.");
    }

    if (!normalizedQuoteNumber.matches("[A-Z0-9_-]+")) {
      throw new IllegalArgumentException(
          "Il numero preventivo può contenere solo lettere, numeri, trattini e underscore.");
    }
  ...
  private static List<PricingLine> validateLines(List<PricingLine> lines) {
    ...
    boolean hasPositiveChargeLine = lines.stream().anyMatch(PricingLine::increasesTotal);

    if (!hasPositiveChargeLine) {
      throw new IllegalArgumentException(
          "Il preventivo deve avere almeno una voce che aumenta il totale.");
    }

    long uniqueCodes = lines.stream().map(PricingLine::getLineCode).distinct().count();
  ...
  public List<PricingLine> getChargeLines() {
    return lines.stream().filter(PricingLine::increasesTotal).toList();
  }

  public List<PricingLine> getDiscountLines() {
    return lines.stream().filter(PricingLine::isDiscount).toList();
  }
