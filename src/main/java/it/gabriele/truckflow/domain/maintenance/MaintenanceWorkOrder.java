...
public final class MaintenanceWorkOrder {
  ...
  private MaintenanceWorkOrder(
    ...
    if (status.blocksVehicleAvailability() && plannedDateRange == null) {
      throw new IllegalArgumentException(
          "Una manutenzione programmata o in corso deve avere un intervallo date.");
    }
  ...
  public static MaintenanceWorkOrder open(
      String workOrderNumber, Vehicle vehicle, MaintenanceType type, Notes notes) {
    return new MaintenanceWorkOrder(
        workOrderNumber, vehicle, type, MaintenanceStatus.OPEN, null, notes);
  }
  ...
  public static MaintenanceWorkOrder scheduled(
      ...
      Notes notes) {
    return new MaintenanceWorkOrder(
        workOrderNumber, vehicle, type, MaintenanceStatus.SCHEDULED, plannedDateRange, notes);
  }
  ...
  private static String validateWorkOrderNumber(String workOrderNumber) {
    ...
    if (normalizedWorkOrderNumber.length() > MAX_WORK_ORDER_NUMBER_LENGTH) {
      throw new IllegalArgumentException(
          "Il numero ordine manutenzione non può superare "
    ...
    if (!normalizedWorkOrderNumber.matches("[A-Z0-9_-]+")) {
      throw new IllegalArgumentException(
          "Il numero ordine manutenzione può contenere solo lettere, numeri, trattini e"
              + " underscore.");
    }
  ...
  public MaintenanceWorkOrder start() {
    ...
    return new MaintenanceWorkOrder(
        workOrderNumber, vehicle, type, MaintenanceStatus.IN_PROGRESS, plannedDateRange, notes);
  }
  ...
  public MaintenanceWorkOrder complete() {
    ...
    return new MaintenanceWorkOrder(
        workOrderNumber, vehicle, type, MaintenanceStatus.COMPLETED, plannedDateRange, notes);
  }
  ...
  public MaintenanceWorkOrder cancel() {
    ...
    return new MaintenanceWorkOrder(
        workOrderNumber, vehicle, type, MaintenanceStatus.CANCELLED, plannedDateRange, notes);
  }
  ...
  public String formatSingleLine() {
    return workOrderNumber + " - " + vehicle.getFleetNumber() + " - " + type + " - " + status;
  }
