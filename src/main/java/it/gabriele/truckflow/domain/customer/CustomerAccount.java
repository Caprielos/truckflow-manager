...
/**
 * Rappresenta un cliente con i suoi contatti. Serve per gestire il cliente come unità operativa
 * completa.
 */
public final class CustomerAccount {
  ...
  private CustomerAccount(Customer customer, List<CustomerContact> contacts) {
    ...
    long primaryContactCount = contacts.stream().filter(CustomerContact::isPrimaryContact).count();

    if (primaryContactCount != 1) {
      throw new IllegalArgumentException(
          "Il cliente deve avere esattamente un contatto principale.");
    }
  ...
  public static CustomerAccount of(
      Customer customer, CustomerContact primaryContact, CustomerContact... otherContacts) {
  ...
  public List<CustomerContact> getContactsByRole(CustomerContactRole role) {
    ...
    return contacts.stream().filter(contact -> contact.hasRole(role)).toList();
  }
  ...
  public boolean equals(Object o) {
    ...
    return customer.equals(that.customer) && contacts.equals(that.contacts);
  }
