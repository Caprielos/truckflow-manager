...
/**
 * Rappresenta un cliente del sistema logistico.
 * Un cliente può richiedere spedizioni, ritiri e consegne.
 */
public final class Customer {
    ...
    private static String validateCode(String code) {
        ...
        if (normalizedCode.length() > MAX_CODE_LENGTH) {
            throw new IllegalArgumentException("Il codice cliente non può superare " + MAX_CODE_LENGTH + " caratteri.");
        }

        if (!normalizedCode.matches("[A-Z0-9_-]+")) {
            throw new IllegalArgumentException("Il codice cliente può contenere solo lettere, numeri, trattini e underscore.");
        }
    ...
    private static String validateLegalName(String legalName) {
        ...
        if (normalizedLegalName.length() > MAX_LEGAL_NAME_LENGTH) {
            throw new IllegalArgumentException("La ragione sociale del cliente non può superare " + MAX_LEGAL_NAME_LENGTH + " caratteri.");
        }
