...
public final class ReportingRules {
  ...
  public static boolean canBeGenerated(GeneratedReport report) {
    ...
    return report.getStatus() == ReportStatus.DRAFT || report.getStatus() == ReportStatus.FAILED;
  }
  ...
  public static boolean canBeFailed(GeneratedReport report) {
    ...
    return report.getStatus() == ReportStatus.DRAFT || report.getStatus() == ReportStatus.GENERATED;
  }

  public static boolean isReadyForPublication(GeneratedReport report) {
    validateReport(report);

    return report.isGenerated() && report.hasGeneratedAt() && report.getMetricCount() > 0;
  }

  public static boolean containsFinancialMetrics(GeneratedReport report) {
    validateReport(report);

    return report.getMetrics().stream().anyMatch(ReportMetric::isMonetary);
  }
  ...
  public static boolean requiresRestrictedAccess(GeneratedReport report) {
    ...
    return report.getDefinition().requiresRestrictedAccess() || containsFinancialMetrics(report);
  }

  public static boolean containsMetricType(GeneratedReport report, ReportMetricType type) {
    validateReport(report);
  ...
  public static BigDecimal calculateMetricTotal(GeneratedReport report, ReportMetricType type) {
    validateReport(report);
