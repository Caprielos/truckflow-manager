...
public final class ReportMetric {
  ...
  public static ReportMetric ofDefaultUnit(
      String metricCode, ReportMetricType type, String label, String value, Notes notes) {
    if (type == null) {
      throw new IllegalArgumentException("Il tipo metrica report è obbligatorio.");
    }

    return new ReportMetric(metricCode, type, label, value, type.getDefaultUnit(), notes);
  }
  ...
  private static String validateCode(String code, String nullMessage) {
    ...
    if (normalizedCode.length() > MAX_CODE_LENGTH) {
      throw new IllegalArgumentException(
          "Il codice metrica report non può superare " + MAX_CODE_LENGTH + " caratteri.");
    }

    if (!normalizedCode.matches("[A-Z0-9_.-]+")) {
      throw new IllegalArgumentException(
          "Il codice metrica report può contenere solo lettere, numeri, punti, trattini e"
              + " underscore.");
    }
  ...
  private static String validateLabel(String label) {
    ...
    if (normalizedLabel.length() > MAX_LABEL_LENGTH) {
      throw new IllegalArgumentException(
          "L'etichetta metrica report non può superare " + MAX_LABEL_LENGTH + " caratteri.");
    }
  ...
  private static String validateUnit(String unit) {
    ...
    if (normalizedUnit.length() > MAX_UNIT_LENGTH) {
      throw new IllegalArgumentException(
          "L'unità metrica report non può superare " + MAX_UNIT_LENGTH + " caratteri.");
    }
  ...
  public String formatSingleLine() {
    return metricCode + " - " + type + " - " + value.toPlainString() + " " + unit;
  }
