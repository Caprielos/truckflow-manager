...
public final class GeneratedReport {
  ...
  public static GeneratedReport draft(
      String reportNumber, ReportDefinition definition, Notes notes) {
    return new GeneratedReport(
        reportNumber, definition, List.of(), ReportStatus.DRAFT, null, notes);
  }
  ...
  public static GeneratedReport generated(
      ...
      Notes notes) {
    return new GeneratedReport(
        reportNumber, definition, metrics, ReportStatus.GENERATED, generatedAt, notes);
  }
  ...
  private static String validateReportNumber(String reportNumber) {
    ...
    if (normalizedReportNumber.length() > MAX_REPORT_NUMBER_LENGTH) {
      throw new IllegalArgumentException(
          "Il numero report generato non può superare " + MAX_REPORT_NUMBER_LENGTH + " caratteri.");
    }

    if (!normalizedReportNumber.matches("[A-Z0-9_-]+")) {
      throw new IllegalArgumentException(
          "Il numero report generato può contenere solo lettere, numeri, trattini e underscore.");
    }
  ...
  private static List<ReportMetric> validateMetrics(
      List<ReportMetric> metrics, ReportStatus status) {
    if (metrics == null) {
    ...
    if (metrics.stream().anyMatch(Objects::isNull)) {
      throw new IllegalArgumentException(
          "La lista metriche report non può contenere valori nulli.");
    }

    boolean mustHaveMetrics =
    ...
    boolean mustBeEmpty = status == ReportStatus.DRAFT || status == ReportStatus.FAILED;

    if (mustBeEmpty && !metrics.isEmpty()) {
      throw new IllegalArgumentException(
          "Un report draft o fallito non deve contenere metriche generate.");
    }

    long uniqueMetricCodes = metrics.stream().map(ReportMetric::getMetricCode).distinct().count();
  ...
  private static void validateGeneratedAt(ReportStatus status, Instant generatedAt) {
    boolean requiresGeneratedAt =
        status == ReportStatus.GENERATED
            || status == ReportStatus.PUBLISHED
            || status == ReportStatus.ARCHIVED;
    ...
    if (requiresGeneratedAt && generatedAt == null) {
      throw new IllegalArgumentException(
          "Un report generato deve avere data e ora di generazione.");
    }

    if (!requiresGeneratedAt && generatedAt != null) {
      throw new IllegalArgumentException(
          "Un report non generato non può avere data e ora di generazione.");
    }
  }

  public GeneratedReport generate(List<ReportMetric> metrics, Instant generatedAt) {
  ...
  public GeneratedReport publish() {
    ...
    return new GeneratedReport(
        reportNumber, definition, metrics, ReportStatus.PUBLISHED, generatedAt, notes);
  }
  ...
  public GeneratedReport archive() {
    ...
    return new GeneratedReport(
        reportNumber, definition, metrics, ReportStatus.ARCHIVED, generatedAt, notes);
  }
  ...
  public GeneratedReport fail() {
    ...
    return new GeneratedReport(
        reportNumber, definition, List.of(), ReportStatus.FAILED, null, notes);
  }
  ...
  public List<ReportMetric> getMetricsByType(ReportMetricType type) {
    ...
    return metrics.stream().filter(metric -> metric.getType() == type).toList();
  }
