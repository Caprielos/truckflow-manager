...
public final class ReportDefinition {
  ...
  private ReportDefinition(
    ...
    this.requestedByAccountId =
        validateCode(requestedByAccountId, "L'account richiedente report è obbligatorio.");
  ...
  public static ReportDefinition of(
      ...
      String requestedByAccountId,
      Notes notes) {
    return new ReportDefinition(reportCode, type, format, period, requestedByAccountId, notes);
  }
  ...
  private static String validateCode(String code, String nullMessage) {
    ...
    if (normalizedCode.length() > MAX_CODE_LENGTH) {
      throw new IllegalArgumentException(
          "Il codice report non può superare " + MAX_CODE_LENGTH + " caratteri.");
    }

    if (!normalizedCode.matches("[A-Z0-9_-]+")) {
      throw new IllegalArgumentException(
          "Il codice report può contenere solo lettere, numeri, trattini e underscore.");
    }
  ...
  public String formatSingleLine() {
    return reportCode + " - " + type + " - " + format + " - requested by: " + requestedByAccountId;
  }
