...
class AvailabilityRulesTest {
  ...
  void shouldConfirmResourceAvailableWhenAvailableAndNotBlocked() {
    ...
    assertTrue(
        AvailabilityRules.isResourceAvailableForWindow(
            records,
  ...
  void shouldRejectResourceWhenBlockingRecordOverlaps() {
    List<ResourceAvailability> records =
        List.of(
            driverAvailable(),
    ...
    assertTrue(
        AvailabilityRules.hasBlockingRecordForWindow(
            records,
            AvailabilityResourceType.DRIVER,
            "DRV-001",
            dateRange(),
  ...
  void shouldCheckIfNewAvailabilityRecordCanBeAdded() {
    ...
    ResourceAvailability anotherAvailable =
    ...
    assertTrue(AvailabilityRules.canAddAvailabilityRecord(List.of(available), anotherAvailable));

    assertFalse(AvailabilityRules.canAddAvailabilityRecord(List.of(available), blocking));
  }
  ...
  void shouldFindRecordsForResource() {
    ...
    List<ResourceAvailability> driverRecords =
        AvailabilityRules.findRecordsForResource(
            records, AvailabilityResourceType.DRIVER, "drv-001");
  ...
  void shouldNotAllowNullValues() {
    ...
    assertThrows(
        ...
        () ->
            AvailabilityRules.isResourceAvailableForWindow(
                null, AvailabilityResourceType.DRIVER, "DRV-001", dateRange(), timeWindow()));

    assertThrows(
        IllegalArgumentException.class,
        () ->
            AvailabilityRules.isResourceAvailableForWindow(
                records, null, "DRV-001", dateRange(), timeWindow()));

    assertThrows(
        IllegalArgumentException.class,
        () ->
            AvailabilityRules.isResourceAvailableForWindow(
                records, AvailabilityResourceType.DRIVER, null, dateRange(), timeWindow()));

    assertThrows(
        IllegalArgumentException.class,
        () ->
            AvailabilityRules.isResourceAvailableForWindow(
                records, AvailabilityResourceType.DRIVER, "DRV-001", null, timeWindow()));

    assertThrows(
        IllegalArgumentException.class,
        () ->
            AvailabilityRules.isResourceAvailableForWindow(
                records, AvailabilityResourceType.DRIVER, "DRV-001", dateRange(), null));

    assertThrows(
        IllegalArgumentException.class,
        () -> AvailabilityRules.canAddAvailabilityRecord(records, null));
  }
  ...
  void shouldNotAllowNullRecordsInsideList() {
    ...
    assertThrows(
        ...
        () ->
            AvailabilityRules.findRecordsForResource(
                recordsWithNull, AvailabilityResourceType.DRIVER, "DRV-001"));
  }
