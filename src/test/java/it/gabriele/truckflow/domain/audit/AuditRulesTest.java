...
class AuditRulesTest {
  ...
  void shouldAllowAppendingChronologicalEventForSameAggregate() {
    AuditTrail trail =
        AuditTrail.of(event("AUD-001", AuditActionType.CREATED, "2026-07-01T08:00:00Z"));

    AuditEvent nextEvent = event("AUD-002", AuditActionType.STATUS_CHANGED, "2026-07-01T09:00:00Z");
  ...
  void shouldRejectAppendingEventBeforeLatestEvent() {
    ...
    AuditEvent oldEvent = event("AUD-003", AuditActionType.STATUS_CHANGED, "2026-07-01T09:00:00Z");
  ...
  void shouldNotAllowNullValues() {
    ...
    assertThrows(
        IllegalArgumentException.class, () -> AuditRules.requiresReview((AuditEvent) null));
    assertThrows(
        IllegalArgumentException.class, () -> AuditRules.requiresReview((AuditTrail) null));
    assertThrows(
        IllegalArgumentException.class, () -> AuditRules.containsSecuritySensitiveEvents(null));
    assertThrows(
        IllegalArgumentException.class, () -> AuditRules.containsFinancialImpactEvents(null));
