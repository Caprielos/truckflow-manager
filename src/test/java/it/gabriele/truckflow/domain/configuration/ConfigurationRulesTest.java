...
class ConfigurationRulesTest {
  ...
  void shouldCheckIfConfigurationCanBeApplied() {
    ...
    SystemConfiguration inactive =
        SystemConfiguration.inactiveForScope(
  ...
  void shouldAllowScopedConfigurationToOverrideGlobalConfiguration() {
    assertTrue(
        ConfigurationRules.canOverride(
            globalPricingConfiguration(), customerPricingConfiguration()));
  }

  @Test
  void shouldRejectInvalidOverride() {
    SystemConfiguration differentType =
        SystemConfiguration.activeForScope(
            "PRICING.FUEL_SURCHARGE_PERCENTAGE",
            ConfigurationCategory.PRICING,
            ConfigurationScope.CUSTOMER,
            "CUST-001",
    ...
    SystemConfiguration differentKey =
        SystemConfiguration.activeForScope(
    ...
    assertFalse(
        ConfigurationRules.canOverride(
            customerPricingConfiguration(), globalPricingConfiguration()));
  ...
  void shouldCheckApplicabilityToScope() {
    assertTrue(
        ConfigurationRules.isApplicableTo(
            globalPricingConfiguration(), ConfigurationScope.CUSTOMER, "CUST-001"));

    assertTrue(
        ConfigurationRules.isApplicableTo(
            customerPricingConfiguration(), ConfigurationScope.CUSTOMER, "cust-001"));

    assertFalse(
        ConfigurationRules.isApplicableTo(
            customerPricingConfiguration(), ConfigurationScope.CUSTOMER, "CUST-999"));
  }
  ...
  void shouldDetectSensitiveTokenConfiguration() {
    SystemConfiguration token =
        SystemConfiguration.activeGlobal(
  ...
  void shouldNotAllowNullValues() {
    ...
    assertThrows(
        IllegalArgumentException.class, () -> ConfigurationRules.canOverride(null, configuration));
    assertThrows(
        IllegalArgumentException.class, () -> ConfigurationRules.canOverride(configuration, null));
    assertThrows(
        IllegalArgumentException.class,
        () -> ConfigurationRules.isApplicableTo(null, ConfigurationScope.CUSTOMER, "CUST-001"));
    assertThrows(
        IllegalArgumentException.class,
        () -> ConfigurationRules.isApplicableTo(configuration, null, "CUST-001"));
    assertThrows(
        IllegalArgumentException.class, () -> ConfigurationRules.isSensitiveConfiguration(null));
    assertThrows(
        IllegalArgumentException.class, () -> ConfigurationRules.requiresRestrictedAccess(null));
    assertThrows(
        IllegalArgumentException.class, () -> ConfigurationRules.isNumericConfiguration(null));
    assertThrows(
        IllegalArgumentException.class, () -> ConfigurationRules.isPricingConfiguration(null));
    assertThrows(
        IllegalArgumentException.class, () -> ConfigurationRules.isSecurityConfiguration(null));
  }
