...
class PricingLineTest {
    ...
    void shouldCreateSurchargeLine() {
        PricingLine line = PricingLine.surcharge(
                "LINE-002",
    ...
    void shouldCreateVehicleWearChargeFromEstimate() {
        PricingLine line = PricingLine.vehicleWearFromEstimate(
                "LINE-WEAR",
                estimate(),
                Notes.empty()
        );
    ...
    void shouldCreateFuelAndTollChargesFromEstimate() {
        PricingLine fuel = PricingLine.fuelFromEstimate(
                "LINE-FUEL",
                estimate(),
                Notes.empty()
        );

        PricingLine toll = PricingLine.tollsFromEstimate(
                "LINE-TOLL",
                estimate(),
                Notes.empty()
        );
    ...
    void shouldCreateDiscountLine() {
        PricingLine line = PricingLine.discount(
                "LINE-003",
                "Sconto cliente",
                Money.of("80.00", "EUR"),
                Notes.empty()
    ...
    void shouldNotCreateSurchargeWithNonSurchargeType() {
        assertThrows(IllegalArgumentException.class, () -> PricingLine.surcharge(
                "LINE-002",
    ...
    void shouldNotCreateEstimateLineWithoutEstimate() {
        assertThrows(IllegalArgumentException.class, () -> PricingLine.fuelFromEstimate(
                "LINE-FUEL",
                null,
                Notes.empty()
        ));
    ...
    void shouldNormalizeLineCodeAndDescription() {
        PricingLine line = PricingLine.of(
                "  line_001  ",
                PricingLineType.BASE_FREIGHT,
                "  Trasporto base  ",
    ...
    void shouldRejectInvalidLineCode() {
        assertThrows(IllegalArgumentException.class, () -> PricingLine.of(
                null,
                PricingLineType.BASE_FREIGHT,
                "Trasporto base",
                Money.of("1000.00", "EUR"),
    ...
    void shouldFormatSingleLine() {
        assertEquals(
                "LINE-001 - BASE_FREIGHT - 1000.00 EUR",
                baseFreightLine().formatSingleLine()
        );
    }
