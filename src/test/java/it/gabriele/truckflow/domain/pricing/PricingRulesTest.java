...
class PricingRulesTest {
  ...
  void shouldNotAllowNullValues() {
    ...
    assertThrows(
        ...
        () -> PricingRules.requiresTemperatureControlSurcharge(null));
    assertThrows(
        IllegalArgumentException.class, () -> PricingRules.requiresInternationalSurcharge(null));
    ...
    assertThrows(
        IllegalArgumentException.class,
        () -> PricingRules.hasTemperatureControlSurchargeLine(null));
  ...
  private static Shipment internationalShipment() {
    return Shipment.fromAcceptedOrder("SHP-002", acceptedInternationalOrder(), Notes.empty());
  }
