...
class TransportClaimTest {
  ...
  void shouldNotAllowInvalidLifecycleTransitions() {
    ...
    TransportClaim settled = underReview.accept(Money.of("800.00", "EUR")).settle(closedDate());
  ...
  void shouldRejectAcceptedCompensationGreaterThanRequested() {
    ...
    assertThrows(
        IllegalArgumentException.class, () -> underReview.accept(Money.of("1500.00", "EUR")));
  }
  ...
  void shouldRejectClosedDateBeforeOpenedDate() {
    ...
    assertThrows(
        IllegalArgumentException.class, () -> underReview.reject(LocalDate.of(2026, 6, 30)));
  }

  @Test
  void shouldDetectTypeAndSeverityDetails() {
    TransportClaim delay =
        TransportClaim.open(
            "CLM-002",
            "SHP-001",
            "CUST-001",
