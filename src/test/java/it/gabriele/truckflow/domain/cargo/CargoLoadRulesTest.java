...
class CargoLoadRulesTest {
  ...
  void shouldDetectFragileCargo() {
    CargoLoad cargoLoad =
        CargoLoad.of(item("Vetri", CargoCategory.FRAGILE, 80, Dimension.ofMeters(1, 1, 1)));
  ...
  void shouldNotAllowNullCargoLoad() {
    ...
    assertThrows(
        IllegalArgumentException.class, () -> CargoLoadRules.containsHazardousMaterial(null));

    assertThrows(IllegalArgumentException.class, () -> CargoLoadRules.containsFragileCargo(null));

    assertThrows(IllegalArgumentException.class, () -> CargoLoadRules.containsOversizedCargo(null));
  }

  @Test
  void shouldNotAllowNullLimits() {
    CargoLoad cargoLoad =
        CargoLoad.of(item("Bancale", CargoCategory.GENERAL, 100, Dimension.ofMeters(1, 1, 1)));

    assertThrows(
        IllegalArgumentException.class, () -> CargoLoadRules.isWithinMaxWeight(cargoLoad, null));

    assertThrows(
        IllegalArgumentException.class, () -> CargoLoadRules.isWithinMaxVolume(cargoLoad, null));

    assertThrows(
        IllegalArgumentException.class, () -> CargoLoadRules.fitsInsideCargoSpace(cargoLoad, null));
  }

  private static CargoItem item(
      String description, CargoCategory category, double kilograms, Dimension dimension) {
    return CargoItem.of(
