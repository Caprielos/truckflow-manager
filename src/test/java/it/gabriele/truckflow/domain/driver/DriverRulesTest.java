...
class DriverRulesTest {
  ...
  void shouldAllowDriverWithBToDriveVan() {
    ...
    assertTrue(DriverRules.hasRequiredLicenseForVehicleCombination(driver, singleVanCombination()));
  }
  ...
  void shouldAllowDriverWithCToDriveRigidTruck() {
    ...
    assertTrue(
        DriverRules.hasRequiredLicenseForVehicleCombination(driver, singleRigidTruckCombination()));
  }

  @Test
  void shouldRequireCAndEForVehicleCombinationWithTrailer() {
    Driver driverWithOnlyC =
        Driver.available(
            "DRV-001",
            "Mario Rossi",
            Set.of(DriverLicenseCategory.C),
    ...
    assertFalse(
        DriverRules.hasRequiredLicenseForVehicleCombination(driverWithOnlyC, trailerCombination()));

    assertTrue(
        DriverRules.hasRequiredLicenseForVehicleCombination(driverWithCAndE, trailerCombination()));
  }

  @Test
  void shouldRequireGoodsCqcForProfessionalGoodsTransport() {
    Driver driverWithoutCqc =
        Driver.available(
            "DRV-001",
            "Mario Rossi",
    ...
    assertFalse(
        DriverRules.hasRequiredProfessionalQualificationForGoodsTransport(driverWithoutCqc));
  ...
  void shouldCheckIfDriverCanDriveVehicleCombination() {
    assertTrue(DriverRules.canDriveVehicleCombination(standardDriver(), trailerCombination()));

    assertFalse(DriverRules.canDriveVehicleCombination(assignedDriver(), trailerCombination()));

    assertFalse(DriverRules.canDriveVehicleCombination(driverWithoutCqc(), trailerCombination()));
  }
  ...
  void shouldRequireAdrBasicForHazardousCargo() {
    ...
    assertTrue(DriverRules.hasRequiredAdrCertificatesForCargoLoad(standardDriver(), cargoLoad));

    assertFalse(DriverRules.hasRequiredAdrCertificatesForCargoLoad(driverWithoutAdr(), cargoLoad));
  }
  ...
  void shouldRequireAdrTankForTankCargo() {
    ...
    assertFalse(
        DriverRules.hasRequiredAdrCertificatesForCargoLoad(driverWithAdrBasicOnly(), cargoLoad));
  }
  ...
  void shouldRequireExplosivesAdrCertificateForExplosives() {
    ...
    assertTrue(
        DriverRules.hasRequiredAdrCertificatesForCargoLoad(driverWithExplosivesAdr(), cargoLoad));

    assertFalse(DriverRules.hasRequiredAdrCertificatesForCargoLoad(standardDriver(), cargoLoad));
  }
  ...
  void shouldRequireRadioactiveAdrCertificateForRadioactiveMaterial() {
    ...
    assertTrue(
        DriverRules.hasRequiredAdrCertificatesForCargoLoad(driverWithRadioactiveAdr(), cargoLoad));
  ...
  void shouldRequireTemperatureQualificationForRefrigeratedShipment() {
    assertTrue(
        DriverRules.hasRequiredOperationalQualificationsForShipment(
            standardDriver(), refrigeratedShipment()));

    assertFalse(
        DriverRules.hasRequiredOperationalQualificationsForShipment(
            driverWithoutTemperatureQualification(), refrigeratedShipment()));
  }
  ...
  void shouldRequireInternationalQualificationForInternationalShipment() {
    assertTrue(
        DriverRules.hasRequiredOperationalQualificationsForShipment(
            standardDriver(), internationalShipment()));

    assertFalse(
        DriverRules.hasRequiredOperationalQualificationsForShipment(
            driverWithoutInternationalQualification(), internationalShipment()));
  }

  @Test
  void shouldCheckIfDriverCanBeAssignedToStandardShipment() {
    assertTrue(
        DriverRules.canBeAssignedToShipment(
            standardDriver(), trailerCombination(), standardShipment()));
  }
  ...
  void shouldNotAssignDriverWithoutAdrToAdrShipment() {
    assertFalse(
        DriverRules.canBeAssignedToShipment(
            driverWithoutAdr(), trailerCombination(), adrTankShipment()));
  }
  ...
  void shouldAssignQualifiedDriverToAdrTankShipment() {
    assertTrue(
        DriverRules.canBeAssignedToShipment(
            standardDriver(), trailerCombination(), adrTankShipment()));
  }
  ...
  private static Driver driverWithExplosivesAdr() {
    return Driver.available(
        ...
        Set.of(DriverLicenseCategory.C, DriverLicenseCategory.E),
        Set.of(DriverProfessionalQualification.CQC_GOODS),
        Set.of(DriverAdrCertificateType.ADR_BASIC, DriverAdrCertificateType.ADR_CLASS_1_EXPLOSIVES),
        Set.of(
            DriverOperationalQualification.INTERNATIONAL_TRANSPORT,
            DriverOperationalQualification.TEMPERATURE_CONTROLLED_TRANSPORT),
        Notes.empty());
  ...
  private static Driver driverWithRadioactiveAdr() {
    return Driver.available(
        ...
        Set.of(DriverProfessionalQualification.CQC_GOODS),
        Set.of(
            DriverAdrCertificateType.ADR_BASIC, DriverAdrCertificateType.ADR_CLASS_7_RADIOACTIVE),
        Set.of(
            DriverOperationalQualification.INTERNATIONAL_TRANSPORT,
            DriverOperationalQualification.TEMPERATURE_CONTROLLED_TRANSPORT),
        Notes.empty());
  ...
  private static VehicleCombination trailerCombination() {
    return VehicleCombination.withTrailer(
        "COMBO-001", tractorUnit(), curtainSideTrailer(), Notes.empty());
  }
  ...
  private static Shipment refrigeratedShipment() {
    return Shipment.fromAcceptedOrder("SHP-002", acceptedRefrigeratedOrder(), Notes.empty());
  }

  private static Shipment internationalShipment() {
    return Shipment.fromAcceptedOrder("SHP-003", acceptedInternationalOrder(), Notes.empty());
  }
