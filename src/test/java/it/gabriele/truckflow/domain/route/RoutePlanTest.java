...
class RoutePlanTest {
  ...
  void shouldNormalizeRouteNumber() {
    RoutePlan routePlan =
        RoutePlan.of("  rte_001  ", standardStops(), Distance.ofKilometers(580), Notes.empty());
  ...
  void shouldNotAllowInvalidRouteNumber() {
    assertThrows(
        ...
        () -> RoutePlan.of(null, standardStops(), Distance.ofKilometers(580), Notes.empty()));

    assertThrows(
        IllegalArgumentException.class,
        () -> RoutePlan.of("RTE 001", standardStops(), Distance.ofKilometers(580), Notes.empty()));
  }
  ...
  void shouldNotAllowTooLongRouteNumber() {
    ...
    assertThrows(
        ...
        () ->
            RoutePlan.of(
                tooLongRouteNumber, standardStops(), Distance.ofKilometers(580), Notes.empty()));
  }
  ...
  void shouldNotAllowNullOrTooShortStopsList() {
    assertThrows(
        IllegalArgumentException.class,
        () -> RoutePlan.of("RTE-001", null, Distance.ofKilometers(580), Notes.empty()));

    assertThrows(
        IllegalArgumentException.class,
        () ->
            RoutePlan.of(
                "RTE-001", List.of(startStop()), Distance.ofKilometers(580), Notes.empty()));
  }
  ...
  void shouldNotAllowNullDistanceOrNotes() {
    assertThrows(
        IllegalArgumentException.class,
        () -> RoutePlan.of("RTE-001", standardStops(), null, Notes.empty()));

    assertThrows(
        IllegalArgumentException.class,
        () -> RoutePlan.of("RTE-001", standardStops(), Distance.ofKilometers(580), null));
  }
  ...
  void shouldNotAllowNonProgressiveSequenceNumbers() {
    ...
    RouteStop endWithWrongSequence =
        RouteStop.of(
            3, RouteStopType.END, romeFacility(), TimeWindow.of("16:00", "17:00"), Notes.empty());
  ...
  void shouldRequireFirstStopToBeStart() {
    ...
    RouteStop end =
        RouteStop.of(
            2, RouteStopType.END, romeFacility(), TimeWindow.of("16:00", "17:00"), Notes.empty());

    assertThrows(
        IllegalArgumentException.class,
        () ->
            RoutePlan.of(
                "RTE-001", List.of(pickup, end), Distance.ofKilometers(580), Notes.empty()));
  }
  ...
  void shouldRequireLastStopToBeEnd() {
    ...
    assertThrows(
        ...
        () ->
            RoutePlan.of(
                "RTE-001", List.of(start, delivery), Distance.ofKilometers(580), Notes.empty()));
  }
  ...
  void shouldReturnUnmodifiableStops() {
    ...
    assertThrows(UnsupportedOperationException.class, () -> routePlan.getStops().add(endStop()));
  }
  ...
  private static RoutePlan standardRoutePlan() {
    return RoutePlan.of("RTE-001", standardStops(), Distance.ofKilometers(580), Notes.empty());
  }

  private static List<RouteStop> standardStops() {
    return List.of(startStop(), pickupStop(), deliveryStop(), endStop());
  }

  private static RouteStop startStop() {
    return RouteStop.of(
        1, RouteStopType.START, milanFacility(), TimeWindow.of("08:00", "09:00"), Notes.empty());
  }

  private static RouteStop pickupStop() {
    return RouteStop.of(
        2, RouteStopType.PICKUP, milanFacility(), TimeWindow.of("09:00", "10:00"), Notes.empty());
  }

  private static RouteStop deliveryStop() {
    return RouteStop.of(
        3, RouteStopType.DELIVERY, romeFacility(), TimeWindow.of("14:00", "15:00"), Notes.empty());
  }

  private static RouteStop endStop() {
    return RouteStop.of(
        4, RouteStopType.END, romeFacility(), TimeWindow.of("16:00", "17:00"), Notes.empty());
  }
