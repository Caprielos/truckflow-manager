...
class GeneratedReportTest {
  ...
  void shouldNormalizeReportNumber() {
    GeneratedReport report = GeneratedReport.draft("  gen_001  ", definition(), Notes.empty());
  ...
  void shouldRejectInvalidReportNumber() {
    assertThrows(
        ...
        () -> GeneratedReport.draft(null, definition(), Notes.empty()));

    assertThrows(
        IllegalArgumentException.class,
        () -> GeneratedReport.draft("GEN 001", definition(), Notes.empty()));
  }
  ...
  void shouldRejectNullMandatoryFields() {
    assertThrows(
        IllegalArgumentException.class,
        () -> GeneratedReport.draft("GEN-001", null, Notes.empty()));

    assertThrows(
        IllegalArgumentException.class, () -> GeneratedReport.draft("GEN-001", definition(), null));

    assertThrows(
        IllegalArgumentException.class,
        () ->
            GeneratedReport.generated("GEN-001", definition(), null, generatedAt(), Notes.empty()));

    assertThrows(
        IllegalArgumentException.class,
        () -> GeneratedReport.generated("GEN-001", definition(), metrics(), null, Notes.empty()));
  }
  ...
  void shouldRejectInvalidMetricList() {
    assertThrows(
        ...
        () ->
            GeneratedReport.generated(
                "GEN-001", definition(), List.of(), generatedAt(), Notes.empty()));
    ...
    assertThrows(
        ...
        () ->
            GeneratedReport.generated(
                "GEN-001", definition(), metricsWithNull, generatedAt(), Notes.empty()));
  ...
  void shouldExposeUnmodifiableMetrics() {
    ...
    assertThrows(
        UnsupportedOperationException.class, () -> report.getMetrics().add(revenueMetric()));
  }
  ...
  void shouldDetectNotes() {
    GeneratedReport report =
        GeneratedReport.draft("GEN-001", definition(), Notes.of("Report da generare"));
  ...
  void shouldFormatSingleLine() {
    assertEquals(
        "GEN-001 - OPERATIONS - GENERATED - metrics: 2", generatedReport().formatSingleLine());
  }
  ...
  private static GeneratedReport draftReport() {
    return GeneratedReport.draft("GEN-001", definition(), Notes.empty());
  }

  private static GeneratedReport generatedReport() {
    return GeneratedReport.generated(
        "GEN-001", definition(), metrics(), generatedAt(), Notes.empty());
  }

  private static ReportDefinition definition() {
    return ReportDefinition.of(
        "RPT-001", ReportType.OPERATIONS, ReportFormat.PDF, period(), "USR-001", Notes.empty());
  }
  ...
  private static List<ReportMetric> metrics() {
    return List.of(
        ...
        ReportMetric.ofDefaultUnit(
            "MET-002", ReportMetricType.SHIPMENT_COUNT, "Spedizioni totali", "25", Notes.empty()));
  }

  private static ReportMetric revenueMetric() {
    return ReportMetric.ofDefaultUnit(
        "MET-001", ReportMetricType.TOTAL_REVENUE, "Ricavi totali", "1250.00", Notes.empty());
  }

  private static DateRange period() {
    return DateRange.of(LocalDate.of(2026, 7, 1), LocalDate.of(2026, 7, 31));
  }
