...
class ReportingRulesTest {
  ...
  void shouldCheckMetricTypeAndCalculateTotal() {
    ...
    assertEquals(
        new BigDecimal("35"),
        ReportingRules.calculateMetricTotal(report, ReportMetricType.SHIPMENT_COUNT));
    assertEquals(
        BigDecimal.ZERO, ReportingRules.calculateMetricTotal(report, ReportMetricType.CLAIM_COUNT));
  }
  ...
  void shouldNotAllowNullValues() {
    ...
    assertThrows(
        IllegalArgumentException.class, () -> ReportingRules.containsFinancialMetrics(null));
    assertThrows(
        IllegalArgumentException.class, () -> ReportingRules.containsSustainabilityMetrics(null));
    assertThrows(
        IllegalArgumentException.class, () -> ReportingRules.requiresRestrictedAccess(null));
    assertThrows(
        IllegalArgumentException.class,
        () -> ReportingRules.containsMetricType(null, ReportMetricType.SHIPMENT_COUNT));
    assertThrows(
        IllegalArgumentException.class, () -> ReportingRules.containsMetricType(report, null));
    assertThrows(
        IllegalArgumentException.class,
        () -> ReportingRules.calculateMetricTotal(null, ReportMetricType.SHIPMENT_COUNT));
    assertThrows(
        IllegalArgumentException.class, () -> ReportingRules.calculateMetricTotal(report, null));
  }

  private static GeneratedReport draftReport() {
    return GeneratedReport.draft("GEN-001", operationsDefinition(), Notes.empty());
  }
    ...
    return GeneratedReport.generated(
        ...
        List.of(
            ReportMetric.ofDefaultUnit(
                ...
                Notes.empty()),
            ReportMetric.ofDefaultUnit(
                "MET-002", ReportMetricType.TOTAL_COST, "Costi totali", "800.00", Notes.empty())),
        generatedAt(),
        Notes.empty());
  }
  ...
  private static ReportDefinition operationsDefinition() {
    return ReportDefinition.of(
        "RPT-OPS", ReportType.OPERATIONS, ReportFormat.PDF, period(), "USR-001", Notes.empty());
  }

  private static ReportDefinition financialDefinition() {
    return ReportDefinition.of(
        "RPT-FIN", ReportType.FINANCIAL, ReportFormat.XLSX, period(), "USR-001", Notes.empty());
  }

  private static ReportDefinition sustainabilityDefinition() {
    return ReportDefinition.of(
        "RPT-SUS", ReportType.SUSTAINABILITY, ReportFormat.PDF, period(), "USR-001", Notes.empty());
  }
