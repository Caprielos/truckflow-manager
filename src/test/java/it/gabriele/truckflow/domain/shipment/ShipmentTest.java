...
class ShipmentTest {
  ...
  void shouldCreateShipmentFromAcceptedOrder() {
    ...
    Shipment shipment = Shipment.fromAcceptedOrder("shp-001", acceptedOrder, Notes.empty());
  ...
  void shouldNotCreateShipmentFromNonAcceptedOrder() {
    ...
    assertThrows(
        ...
        () -> Shipment.fromAcceptedOrder("SHP-001", draftOrder, Notes.empty()));
  }

  @Test
  void shouldNormalizeShipmentNumber() {
    Shipment shipment =
        Shipment.fromAcceptedOrder("  shp_001  ", acceptedStandardOrder(), Notes.empty());
  ...
  void shouldNotAllowInvalidShipmentNumber() {
    assertThrows(
        IllegalArgumentException.class,
        () -> Shipment.fromAcceptedOrder(null, acceptedStandardOrder(), Notes.empty()));

    assertThrows(
        IllegalArgumentException.class,
        () -> Shipment.fromAcceptedOrder("SHP 001", acceptedStandardOrder(), Notes.empty()));
  }
  ...
  void shouldNotAllowNullMandatoryFields() {
    assertThrows(
        IllegalArgumentException.class,
        () -> Shipment.fromAcceptedOrder("SHP-001", null, Notes.empty()));

    assertThrows(
        IllegalArgumentException.class,
        () -> Shipment.fromAcceptedOrder("SHP-001", acceptedStandardOrder(), null));
  }
  ...
  void shouldDispatchPlannedShipment() {
    Shipment dispatched = createdShipment().plan().dispatch();
  ...
  void shouldMarkDispatchedShipmentInTransit() {
    Shipment inTransit = createdShipment().plan().dispatch().markInTransit();
  ...
  void shouldDeliverInTransitShipment() {
    Shipment delivered = createdShipment().plan().dispatch().markInTransit().deliver();
  ...
  private static TransportOrder acceptedStandardOrder() {
    return draftStandardOrder().submit().accept();
  }
