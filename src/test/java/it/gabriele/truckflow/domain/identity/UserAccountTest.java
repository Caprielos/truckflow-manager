...
class UserAccountTest {
  ...
  void shouldRejectNullPermissionSetOrNullPermission() {
    ...
    Set<UserPermission> permissionsWithNull =
        new HashSet<>(Arrays.asList(UserPermission.MANAGE_USERS, null));
  ...
  void shouldCheckRolesAndPermissions() {
    ...
    assertTrue(
        account.hasAnyPermission(Set.of(UserPermission.MANAGE_USERS, UserPermission.VIEW_REPORTS)));
  ...
  void shouldFormatSingleLine() {
    assertEquals(
        "USR-001 - admin@example.com - ACTIVE - roles: 1", adminAccount().formatSingleLine());
  }
