...
class RealisticFleetModelTest {
  ...
  void shouldCalculatePayloadWeightClassAndEpalCapacity() {
    VehicleMassSpecification mass =
        VehicleMassSpecification.of(
            Weight.ofKilograms(44000), Weight.ofKilograms(15000), Weight.ofKilograms(30000), null);

    VehicleDimensionSpecification dimensions =
        VehicleDimensionSpecification.of(
            Dimension.ofMeters(13.6, 2.55, 4.0), Dimension.ofMeters(13.6, 2.48, 2.7), 1.2, null);
  ...
  void shouldModelCurtainSideMegaWithTailLift() {
    VehicleBodyConfiguration configuration =
        VehicleBodyConfiguration.of(
            VehicleBodyBaseType.CURTAIN_SIDE,
            List.of(
                VehicleLoadingEquipment.of(
  ...
  void shouldCalculateVehicleCertificateDeadlineStatus() {
    ...
    assertEquals(
        DeadlineStatus.EXPIRING_SOON, certificate.calculateStatus(LocalDate.of(2026, 6, 10), 30));
  ...
  void shouldCheckAutotrenoGrossWeightAndTowRatio() {
    ...
    assertTrue(
        VehicleCombinationTechnicalRules.isWithinGrossWeightLimit(
            truck, trailer, VehicleCombinationLegalLimitProfile.italianStandardAutotreno()));
  }
}
