...
class MaintenanceWorkOrderTest {
  ...
  void shouldNormalizeWorkOrderNumber() {
    MaintenanceWorkOrder workOrder =
        MaintenanceWorkOrder.open(
            "  mnt_001  ", standardVehicle(), MaintenanceType.ROUTINE_SERVICE, Notes.empty());
  ...
  void shouldRejectInvalidWorkOrderNumber() {
    assertThrows(
        ...
        () ->
            MaintenanceWorkOrder.open(
                null, standardVehicle(), MaintenanceType.ROUTINE_SERVICE, Notes.empty()));

    assertThrows(
        IllegalArgumentException.class,
        () ->
            MaintenanceWorkOrder.open(
                "MNT 001", standardVehicle(), MaintenanceType.ROUTINE_SERVICE, Notes.empty()));
  }
  ...
  void shouldRejectNullMandatoryFields() {
    assertThrows(
        ...
        () ->
            MaintenanceWorkOrder.open(
                "MNT-001", null, MaintenanceType.ROUTINE_SERVICE, Notes.empty()));

    assertThrows(
        IllegalArgumentException.class,
        () -> MaintenanceWorkOrder.open("MNT-001", standardVehicle(), null, Notes.empty()));

    assertThrows(
        IllegalArgumentException.class,
        () ->
            MaintenanceWorkOrder.open(
                "MNT-001", standardVehicle(), MaintenanceType.ROUTINE_SERVICE, null));
  ...
  void shouldNotCancelTerminalWorkOrder() {
    MaintenanceWorkOrder completed = openWorkOrder().schedule(dateRange()).start().complete();
  ...
  void shouldDetectMaintenanceTypeDetails() {
    MaintenanceWorkOrder tireWorkOrder =
        MaintenanceWorkOrder.open(
            "MNT-002", standardVehicle(), MaintenanceType.TIRE_REPLACEMENT, Notes.empty());

    MaintenanceWorkOrder adrWorkOrder =
        MaintenanceWorkOrder.open(
            "MNT-003", standardVehicle(), MaintenanceType.ADR_TANK_INSPECTION, Notes.empty());
  ...
  void shouldFormatSingleLine() {
    assertEquals(
        "MNT-001 - TRUCK-001 - ROUTINE_SERVICE - OPEN", openWorkOrder().formatSingleLine());
  }
  ...
  private static DateRange dateRange() {
    return DateRange.of(LocalDate.of(2026, 7, 1), LocalDate.of(2026, 7, 2));
  }
}
