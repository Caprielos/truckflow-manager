...
class BillingRulesTest {
    ...
    void shouldCheckPaymentMatchingInvoice() {
        assertTrue(BillingRules.isPaymentMatchingInvoice(
                fullPayment(),
                issuedInvoice()
        ));

        assertFalse(BillingRules.isPaymentMatchingInvoice(
                partialPayment(),
                issuedInvoice()
        ));

        assertFalse(BillingRules.isPaymentMatchingInvoice(
                paymentForDifferentInvoice(),
                issuedInvoice()
        ));
    }
    ...
    void shouldCheckIfPaymentCanBeRegistered() {
        assertTrue(BillingRules.canRegisterPayment(
                fullPayment(),
                issuedInvoice()
        ));

        assertFalse(BillingRules.canRegisterPayment(
                fullPayment(),
                draftInvoice()
        ));
    ...
    void shouldCheckIfPaymentsCoverInvoice() {
        ...
        assertFalse(BillingRules.paymentsCoverInvoice(List.of(paymentForDifferentInvoice()), issuedInvoice()));
    }
    ...
    void shouldNotAllowNullValues() {
        ...
        assertThrows(IllegalArgumentException.class, () -> BillingRules.isOverdue(null, LocalDate.of(2026, 8, 1)));
        assertThrows(IllegalArgumentException.class, () -> BillingRules.isOverdue(invoice, null));
        assertThrows(IllegalArgumentException.class, () -> BillingRules.isPaymentMatchingInvoice(null, invoice));
        assertThrows(IllegalArgumentException.class, () -> BillingRules.isPaymentMatchingInvoice(payment, null));
        assertThrows(IllegalArgumentException.class, () -> BillingRules.canRegisterPayment(null, invoice));
        assertThrows(IllegalArgumentException.class, () -> BillingRules.canRegisterPayment(payment, null));
        assertThrows(IllegalArgumentException.class, () -> BillingRules.paymentsCoverInvoice(null, invoice));
        assertThrows(IllegalArgumentException.class, () -> BillingRules.paymentsCoverInvoice(List.of(payment), null));
    }
    ...
    void shouldNotAllowNullPaymentsInsideList() {
        ...
        assertThrows(IllegalArgumentException.class, () -> BillingRules.paymentsCoverInvoice(
                paymentsWithNull,
                issuedInvoice()
        ));
    }
